...
public class RoleInfo
{
    ...
    public virtual string nameColored => Helpers.cs(color, name);
    ...
    //public virtual string roleOptions => GameOptionsDataPatch.optionsToString(baseOption, true);
    ...
    private RoleInfo(string name, Color color, CustomOption baseOption, RoleType roleType)
    {
        this.color = color;
        nameKey = name;
    ...
    public static void Load()
    {
        ...
        morphling = new RoleInfo("morphling", Morphling.color, CustomOptionHolder.morphlingSpawnRate,
            RoleType.Morphling);
        camouflager = new RoleInfo("camouflager", Camouflager.color, CustomOptionHolder.camouflagerSpawnRate,
        ...
        trickster = new RoleInfo("trickster", Trickster.color, CustomOptionHolder.tricksterSpawnRate,
            RoleType.Trickster);
        ...
        bountyHunter = new RoleInfo("bountyHunter", BountyHunter.color, CustomOptionHolder.bountyHunterSpawnRate,
            RoleType.BountyHunter);
        detective = new RoleInfo("detective", Detective.color, CustomOptionHolder.detectiveSpawnRate,
            RoleType.Detective);
        timeMaster = new RoleInfo("timeMaster", TimeMaster.color, CustomOptionHolder.timeMasterSpawnRate,
            RoleType.TimeMaster);
        ...
        corruptedShifter = new RoleInfo("corruptedShifter", Shifter.color, CustomOptionHolder.shifterSpawnRate,
            RoleType.Shifter);
        niceSwapper = new RoleInfo("niceSwapper", Swapper.color, CustomOptionHolder.swapperSpawnRate, RoleType.Swapper);
        evilSwapper = new RoleInfo("evilSwapper", Palette.ImpostorRed, CustomOptionHolder.swapperSpawnRate,
            RoleType.Swapper);
        ...
        securityGuard = new RoleInfo("securityGuard", SecurityGuard.color, CustomOptionHolder.securityGuardSpawnRate,
        ...
        niceGuesser = new RoleInfo("niceGuesser", Guesser.color, CustomOptionHolder.guesserSpawnRate,
            RoleType.NiceGuesser);
        evilGuesser = new RoleInfo("evilGuesser", Palette.ImpostorRed, CustomOptionHolder.guesserSpawnRate,
        ...
        opportunist = new RoleInfo("opportunist", Opportunist.color, CustomOptionHolder.opportunistSpawnRate,
            RoleType.Opportunist);
        ...
        plagueDoctor = new RoleInfo("plagueDoctor", PlagueDoctor.color, CustomOptionHolder.plagueDoctorSpawnRate,
            RoleType.PlagueDoctor);
        nekoKabocha = new RoleInfo("nekoKabocha", NekoKabocha.color, CustomOptionHolder.nekoKabochaSpawnRate,
            RoleType.NekoKabocha);
        niceWatcher = new RoleInfo("niceWatcher", Watcher.color, CustomOptionHolder.watcherSpawnRate, RoleType.Watcher);
        evilWatcher = new RoleInfo("evilWatcher", Palette.ImpostorRed, CustomOptionHolder.watcherSpawnRate,
            RoleType.Watcher);
        serialKiller = new RoleInfo("serialKiller", SerialKiller.color, CustomOptionHolder.serialKillerSpawnRate,
        ...
        fortuneTeller = new RoleInfo("fortuneTeller", FortuneTeller.color, CustomOptionHolder.fortuneTellerSpawnRate,
            RoleType.FortuneTeller);
        akujo = new RoleInfo("akujo", Akujo.color, CustomOptionHolder.akujoSpawnRate, RoleType.Akujo);
        schrodingersCat = new RoleInfo("schrodingersCat", SchrodingersCat.color,
            CustomOptionHolder.schrodingersCatSpawnRate, RoleType.SchrodingersCat);
        ...
        evilTracker = new RoleInfo("evilTracker", EvilTracker.color, CustomOptionHolder.evilTrackerSpawnRate,
            RoleType.EvilTracker);
        puppeteer = new RoleInfo("puppeteer", Puppeteer.color, CustomOptionHolder.puppeteerSpawnRate,
            RoleType.Puppeteer);
        evilHacker = new RoleInfo("evilHacker", EvilHacker.color, CustomOptionHolder.evilHackerSpawnRate,
            RoleType.EvilHacker);
        ...
        jekyllAndHyde = new RoleInfo("jekyllAndHyde", JekyllAndHyde.color, CustomOptionHolder.jekyllAndHydeSpawnRate,
            RoleType.JekyllAndHyde);
        ...
        bool includeHidden = false)
    {
        ...
        {
            if ((includeHidden || PlayerControl.LocalPlayer) &&
                PlayerControl.LocalPlayer.Data.IsDead)
                infos.Add(fortuneTeller);
            else
            {
                RoleInfo info = FortuneTeller.isCompletedNumTasks(p) ? fortuneTeller : crewmate;
                infos.Add(info);
            }
        }
    ...
    public static string GetRolesString(PlayerControl p, bool useColors, RoleType[] excludeRoles = null,
        bool includeHidden = false, string joinSeparator = " ")
    {
        if (p?.Data?.Disconnected != false) return "";

        List<RoleInfo> roleInfo = getRoleInfoForPlayer(p, excludeRoles, includeHidden);
        string roleName = string.Join(joinSeparator,
            roleInfo.Select(x => useColors ? Helpers.cs(x.color, x.name) : x.name).ToArray());
        if (Lawyer.target != null && p?.PlayerId == Lawyer.target.PlayerId &&
            PlayerControl.LocalPlayer != Lawyer.target)
        ...
        if (p.hasModifier(ModifierType.Madmate) || p.hasModifier(ModifierType.CreatedMadmate))
        {
            ...
            else
            {
                ...
                roleName = string.Join(joinSeparator,
                    roleInfo.Select(x => useColors ? Helpers.cs(Madmate.color, x.name) : x.name).ToArray());
        ...
        if (p.hasModifier(ModifierType.LastImpostor))
        {
            ...
            {
                string postfix =
                    useColors ? Helpers.cs(LastImpostor.color, LastImpostor.postfix) : LastImpostor.postfix;
                roleName = string.Join(joinSeparator,
                    roleInfo.Select(x => useColors ? Helpers.cs(x.color, x.name) : x.name).ToArray());
                roleName += postfix;
            }
