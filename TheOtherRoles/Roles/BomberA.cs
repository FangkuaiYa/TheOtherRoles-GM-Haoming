...
public class BomberA : RoleBase<BomberA>
{
    ...
    public static bool countAsOne => CustomOptionHolder.bomberCountAsOne.getBool();
    public static bool showEffects => CustomOptionHolder.bomberShowEffects.getBool();
    public static bool ifOneDiesBothDie => CustomOptionHolder.bomberIfOneDiesBothDie.getBool();
    public static bool hasOneVote => CustomOptionHolder.bomberHasOneVote.getBool();
    public static bool alwaysShowArrow => CustomOptionHolder.bomberAlwaysShowArrow.getBool();
    ...
    public override void FixedUpdate()
    {
        if (player == PlayerControl.LocalPlayer)
        {
            ...
            if (player.isAlive() && BomberB.isAlive())
            {
                ...
                // 相方の設置したターゲットを表示する
                if (BomberB.bombTarget != null && playerIcons.ContainsKey(BomberB.bombTarget.PlayerId) &&
                    playerIcons[BomberB.bombTarget.PlayerId].gameObject != null)
                {
    ...
    public override void OnDeath(PlayerControl killer = null)
    {
        if (ifOneDiesBothDie)
        {
            PlayerControl partner = BomberB.players.FirstOrDefault().player;
    ...
    public static void MakeButtons(HudManager hm)
    {
        ...
        bomberButton = new CustomButton(
            ...
            () =>
            {
                return PlayerControl.LocalPlayer.isRole(RoleType.BomberA) &&
                       PlayerControl.LocalPlayer.isAlive() && BomberB.isAlive();
            },
            // CouldUse
            () =>
            {
        ...
        releaseButton = new CustomButton(
            ...
            () =>
            {
                PlayerControl bomberB = BomberB.allPlayers.FirstOrDefault();
                float distance = Vector2.Distance(PlayerControl.LocalPlayer.transform.localPosition,
                    bomberB.transform.localPosition);

                if (PlayerControl.LocalPlayer.CanMove && bombTarget != null &&
                    BomberB.bombTarget != null && bomberB.isAlive() && distance < 1)
                {
            ...
            () =>
            {
                ...
                return PlayerControl.LocalPlayer.CanMove && bombTarget != null &&
                       BomberB.bombTarget != null && bomberB.isAlive() && distance < 1;
            },
            // OnMeetingEnds
    ...
    {
        foreach (BomberA bomber in players)
