...
public abstract class RoleBase<T> : Role where T : RoleBase<T>, new()
{
    ...
    {
        get { return players.FirstOrDefault(x => x.player == PlayerControl.LocalPlayer); }
    }

    public static List<PlayerControl> allPlayers
    {
        get { return players.Select(x => x.player).ToList(); }
    }

    public static List<PlayerControl> livingPlayers
    {
        get { return players.Select(x => x.player).Where(x => x.isAlive()).ToList(); }
    }
...
public static class RoleHelpers
{
    ...
    {
        foreach (KeyValuePair<RoleType, Type> t in RoleData.allRoleTypes)
            if (role == t.Key)
                return (bool)t.Value.GetMethod("isRole", BindingFlags.Public | BindingFlags.Static)
    ...
    {
        LogHelper.Info($"{player?.Data?.PlayerName}({player?.PlayerId}): {Enum.GetName(typeof(RoleType), role)}",
            "Player.setRole");
        foreach (KeyValuePair<RoleType, Type> t in RoleData.allRoleTypes)
            if (role == t.Key)
            {
                t.Value.GetMethod("setRole", BindingFlags.Public | BindingFlags.Static)
                    ?.Invoke(null, new object[] { player });
                return;
            }
    ...
    public static void eraseRole(this PlayerControl player, RoleType role)
    {
        if (isRole(player, role))
        {
            foreach (KeyValuePair<RoleType, Type> t in RoleData.allRoleTypes)
                if (role == t.Key)
                {
                    t.Value.GetMethod("eraseRole", BindingFlags.Public | BindingFlags.Static)
                        ?.Invoke(null, new object[] { player });
                    return;
                }
    ...
    public static void swapRoles(this PlayerControl player, PlayerControl target)
    {
        foreach (KeyValuePair<RoleType, Type> t in RoleData.allRoleTypes)
            if (player.isRole(t.Key))
                t.Value.GetMethod("swapRole", BindingFlags.Public | BindingFlags.Static)
                    ?.Invoke(null, new object[] { player, target });
    ...
    public static string modifyNameText(this PlayerControl player, string nameText)
    {
        ...
        foreach (Modifier mod in Modifier.allModifiers)
    ...
    public static string modifyRoleText(this PlayerControl player, string roleText, List<RoleInfo> roleInfo,
        bool useColors = true, bool includeHidden = false)
    {
        foreach (Modifier mod in Modifier.allModifiers)
            if (mod.player == player)
