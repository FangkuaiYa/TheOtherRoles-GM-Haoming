...
public class Munou : ModifierBase<Munou>
{
    ...
    public static bool endGameFlag;
    public static bool randomColorFlag;
    ...
    public static int probability => (int)CustomOptionHolder.munouProbability.getFloat();
    public static int numShufflePlayers => (int)CustomOptionHolder.munouNumShufflePlayers.getFloat();
    public static string postfix => ModTranslation.getString("incompetent");

    public static MunouType munouType => (MunouType)CustomOptionHolder.munouType.getSelection();
        ...
        get
        {
            ...
            foreach (PlayerControl player in PlayerControl.AllPlayerControls.GetFastEnumerator())
            {
                ...
                if (info.Contains(RoleInfo.crewmate) && !player.hasModifier(ModifierType.Munou) &&
                    !player.isRole(RoleType.FortuneTeller)) crewNoRole.Add(player);
                if (!player.hasModifier(ModifierType.Munou)) validPlayers.Add(player);
            }

            if (munouType == MunouType.Simple) return crewNoRole;
            if (munouType == MunouType.Random) return validPlayers;
            return validPlayers;
        }
    }
    ...
    public override void OnMeetingEnd()
    {
        if (player == PlayerControl.LocalPlayer &&
            PlayerControl.LocalPlayer.hasModifier(ModifierType.Munou) &&
            PlayerControl.LocalPlayer.isAlive()) randomColors();
    }
    ...
    {
        if (player == PlayerControl.LocalPlayer &&
            PlayerControl.LocalPlayer.hasModifier(ModifierType.Munou)) resetColors();
    }
    ...
    public static void randomColors()
    {
        ...
        List<byte> shuffleTargets = alivePlayers.Count > numShufflePlayers
            ? alivePlayers.Take(numShufflePlayers).ToList()
            : alivePlayers;
        foreach (byte id in shuffleTargets)
        {
            ...
            PlayerControl to = Helpers.playerById(shuffleTargets[rnd]);
            p.morphToPlayer(to);
    ...
    public static void reMorph(byte playerId)
    {
        if (colorPairs.ContainsKey(playerId))
            Helpers.playerById(playerId).morphToPlayer(Helpers.playerById(colorPairs[playerId]));
    }
