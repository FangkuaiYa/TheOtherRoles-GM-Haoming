...
public abstract class Modifier
{
    ...
    public virtual string modifyRoleText(string roleText, List<RoleInfo> roleInfo, bool useColors = true,
...
public abstract class ModifierBase<T> : Modifier where T : ModifierBase<T>, new()
{
    ...
    public static List<RoleType> persistRoleChange = new();
...
public static class ModifierHelpers
{
    ...
    {
        foreach (KeyValuePair<ModifierType, Type> t in ModifierData.allModTypes)
            if (mod == t.Key)
                return (bool)t.Value.GetMethod("hasModifier", BindingFlags.Public | BindingFlags.Static)
    ...
    public static void addModifier(this PlayerControl player, ModifierType mod)
    {
        foreach (KeyValuePair<ModifierType, Type> t in ModifierData.allModTypes)
            if (mod == t.Key)
            {
                t.Value.GetMethod("addModifier", BindingFlags.Public | BindingFlags.Static)
                    ?.Invoke(null, new object[] { player });
                return;
            }
    }
    ...
    public static void eraseModifier(this PlayerControl player, ModifierType mod)
    {
        if (hasModifier(player, mod))
        {
            foreach (KeyValuePair<ModifierType, Type> t in ModifierData.allModTypes)
                if (mod == t.Key)
                {
                    t.Value.GetMethod("eraseModifier", BindingFlags.Public | BindingFlags.Static)
                        ?.Invoke(null, new object[] { player });
                    return;
                }
    ...
    public static void eraseAllModifiers(this PlayerControl player, RoleType newRole = RoleType.NoRole)
    {
        foreach (KeyValuePair<ModifierType, Type> t in ModifierData.allModTypes)
            t.Value.GetMethod("eraseModifier", BindingFlags.Public | BindingFlags.Static)
                ?.Invoke(null, new object[] { player, newRole });
    }
    ...
    public static void swapModifiers(this PlayerControl player, PlayerControl target)
    {
        foreach (KeyValuePair<ModifierType, Type> t in ModifierData.allModTypes)
            if (player.hasModifier(t.Key))
                t.Value.GetMethod("swapModifier", BindingFlags.Public | BindingFlags.Static)
