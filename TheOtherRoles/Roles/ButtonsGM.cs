    ...
    public static void makeButtons(HudManager hm)
    {
        ...
        Action gmButtonOnClick(byte index)
        {
            return () =>
            {
                ...
                if (!TORMapOptions.playerIcons.ContainsKey(index) || target.Data.Disconnected) return;
        ...
        Func<bool> gmHasButton(byte index)
        {
            return () =>
            {
                if (GM.gm == null || PlayerControl.LocalPlayer != GM.gm ||
                    !TORMapOptions.playerIcons.ContainsKey(index) ||
                    !GM.canWarp ||
        ...
        Func<bool> gmCouldUse(byte index)
        {
            return () =>
            {
                if (!TORMapOptions.playerIcons.ContainsKey(index) || !GM.canWarp) return false;
                ...
                if (gmButtons[index].PositionOffset != pos)
                {
                    ...
                    TORMapOptions.playerIcons[index].transform.localPosition = iconBase + pos;
        ...
        Func<bool> gmCouldKill(byte index)
        {
            return () =>
            {
                if (!TORMapOptions.playerIcons.ContainsKey(index) || !GM.canKill) return false;
        ...
        for (byte i = 0; i < 15; i++)
        {
            ...
            Vector3 buttonPos = gmKillButton.actionButton.buttonLabelText.transform.localPosition;
            gmKillButton.actionButton.buttonLabelText.transform.localPosition =
                new Vector3(buttonPos.x, buttonPos.y + 0.6f, -500f);
        ...
        gmZoomOut = new CustomButton(
            () =>
            {
                ...
                if (hm.transform.localScale.x < 6.0f) hm.transform.localScale *= 1.5f;
        ...
        gmZoomIn = new CustomButton(
            () =>
            {
                ...
                if (hm.transform.localScale.x > 1.0f) hm.transform.localScale /= 1.5f;
