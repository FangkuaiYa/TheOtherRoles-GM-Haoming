...
public class FortuneTeller : RoleBase<FortuneTeller>
{
    ...
    public static int numTasks => (int)CustomOptionHolder.fortuneTellerNumTasks.getFloat();
    public static DivineResults divineResult => (DivineResults)CustomOptionHolder.fortuneTellerResults.getSelection();
    public static float duration => CustomOptionHolder.fortuneTellerDuration.getFloat();
    public static float distance => CustomOptionHolder.fortuneTellerDistance.getFloat();
    ...
    {
        bool status = true;
        if (playerStatus.ContainsKey(index)) status = playerStatus[index];
    ...
    public static void MakeButtons(HudManager hm)
    {
        ...
        void setButtonPos(byte index)
        {
            ...
            Vector3 iconBase = new Vector3(-0.82f, 0.19f, 0) + Patches.IntroCutsceneOnDestroyPatch.bottomLeft;
        ...
        Func<bool> fortuneTellerCouldUse(byte index)
        {
            return () =>
            {
                ...
                {
                    if (TORMapOptions.playerIcons.ContainsKey(index))
                        TORMapOptions.playerIcons[index].gameObject.SetActive(false);
                ...
                // ボタンにテキストを設定
                bool status = true;
                if (local.playerStatus.ContainsKey(index)) status = local.playerStatus[index];

                if (status)
                {
                    float progress = local.progress.ContainsKey(index) ? local.progress[index] : 0f;
                ...
                TORMapOptions.playerIcons[index].gameObject
                    .SetActive(Helpers.ShowButtons && PlayerControl.LocalPlayer.CanMove);
                fortuneTellerButtons[index]
                    .setActive(Helpers.ShowButtons && PlayerControl.LocalPlayer.CanMove);
    ...
    private void fortuneTellerUpdate()
    {
        if (player == PlayerControl.LocalPlayer && !meetingFlag)
            foreach (PlayerControl p in PlayerControl.AllPlayerControls)
            {
                ...
                // 障害物判定
                bool anythingBetween = PhysicsHelpers.AnythingBetween(p.GetTruePosition(),
                    fortuneTeller.GetTruePosition(), Constants.ShipAndObjectsMask, false);
    ...
    public void impostorArrowUpdate()
    {
        if (PlayerControl.LocalPlayer.isImpostor())
        {
            ...
            if (updateTimer <= 0.0f)
            {
                ...
                foreach (FortuneTeller p in players)
                {
    ...
    public void divine(PlayerControl p)
    {
        ...
        if (!string.IsNullOrWhiteSpace(msg)) fortuneTellerMessage(msg, 5f, color);
