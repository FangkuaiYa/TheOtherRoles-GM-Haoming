...
public class Puppeteer : RoleBase<Puppeteer>
{
    ...
    public static bool triggerPuppeteerWin;
    public static bool isActive;
    public static bool canSpawn = true;
    public static PlayerControl dummy;
    public static PlayerControl target;
    public static PlayerControl currentTarget;
    public static PlayerControl tmpTarget;
    public static bool stealthed;
    ...
    public static float posUpdateTimer;
    public static AudioClip laugh;
    public static bool soundFlag;

    public static TMP_Text puppeteerText;

    public static float originalZoom;
    ...
    public static int numKills => (int)CustomOptionHolder.puppeteerNumKills.getFloat();
    public static float sampleDuration => CustomOptionHolder.puppeteerSampleDuration.getFloat();
    public static bool canControlDummyEvenIfDead => CustomOptionHolder.puppeteerCanControlDummyEvenIfDead.getBool();
    public static int penaltyOnDeath => (int)CustomOptionHolder.puppeteerPenaltyOnDeath.getFloat();
    public static bool losesSenriganOnDeath => CustomOptionHolder.puppeteerLosesSenriganOnDeath.getBool();

    public override void OnMeetingStart()
    {
        bool isAlive = allPlayers.FindAll(x => x.isAlive()).Count >= 1;
        if (soundFlag && isAlive) SoundManager.Instance.PlaySound(laugh, false);
        soundFlag = false;
        if (!isAlive && (PlayerControl.LocalPlayer.isImpostor() ||
    ...
    public override void OnMeetingEnd()
    {
        ...
        canSpawn = false;
        isActive = false;
        if (PlayerControl.LocalPlayer.isRole(RoleType.Puppeteer)) switchStealth(false);
    }
    ...
    public static Sprite getSampleButtonSprite()
    {
        ...
        sampleButtonSprite = Helpers.loadSpriteFromResources("TheOtherRoles.Resources.SampleButton.png", 115f);
    ...
    public static Sprite getPuppeteerButtonSprite()
    {
        ...
        puppeteerButtonSprite = TheOtherRolesPlugin.getResources("PuppeteerButton.png");
    ...
    public static void MakeButtons(HudManager hm)
    {
        sampleButton = new CustomButton(
            ...
            () =>
            {
                return PlayerControl.LocalPlayer.isRole(RoleType.Puppeteer) &&
                       (PlayerControl.LocalPlayer.isAlive() || canControlDummyEvenIfDead);
            },
            // CouldUse
            () =>
            {
        ...
        puppeteerButton = new CustomButton(
            ...
            () =>
            {
                if (puppeteerText != null) puppeteerText.text = $"{counter}/{numKills}";
                return true;
            },
            // OnMeetingEnds
        ...
        puppeteerText = GameObject.Instantiate(puppeteerButton.actionButton.cooldownTimerText,
            puppeteerButton.actionButton.cooldownTimerText.transform.parent);
        puppeteerText.text = "";
    ...
    public static void spawnDummy()
    {
        ...
        {
            if (p == 1) RPCProcedure.moveDummy(PlayerControl.LocalPlayer.transform.position);
        })));
        if (target != null)
        {
            writer = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId,
    ...
    public static void senrigan(bool toggle)
    {
        ...
        if (!toggle)
        {
            ...
            if (PlayerControl.LocalPlayer.isAlive()) hm.ShadowQuad.gameObject.SetActive(true);
        }
        else
        {
    ...
    public static void switchStealth(bool flag)
    {
        if (!flag)
        {
            ...
            player.LightPrefab = Object.Instantiate(player.LightPrefab);
            player.LightPrefab.transform.SetParent(player.transform);
            player.LightPrefab.transform.localPosition = player.Collider.offset;
        ...
        else
        {
            ...
            dummy.LightPrefab = Object.Instantiate(dummy.LightPrefab);
            dummy.LightPrefab.transform.SetParent(dummy.transform);
            dummy.LightPrefab.transform.localPosition = dummy.Collider.offset;
    ...
    {
        bool isAlive = allPlayers.FindAll(x => x.isAlive()).Count >= 1;
        if (!target.isImpostor() && !target.isRole(RoleType.Jackal) && !target.isRole(RoleType.JekyllAndHyde) &&
            !target.isRole(RoleType.Moriarty) && isAlive) counter += 1;
    ...
    public static void OnDummyDeath(PlayerControl killer)
    {
        ...
        bool isAlive = allPlayers.FindAll(x => x.isAlive()).Count >= 1;
        if (!isAlive) killer.SetKillTimer(0f);
    ...
    private static void arrowUpdate()
    {
        ...
        if (updateTimer <= 0.0f)
        {
            ...
            foreach (PlayerControl p in PlayerControl.AllPlayerControls)
            {
                ...
                Arrow arrow;
                if (p.Data.Role.IsImpostor || p.isRole(RoleType.Jackal) || p.isRole(RoleType.JekyllAndHyde) ||
                    p.isRole(RoleType.Moriarty) || p == target)
                {
                    if (p.Data.Role.IsImpostor)
                        arrow = new Arrow(Color.red);
                    else if (p.isRole(RoleType.Jackal) || (p.isRole(RoleType.SchrodingersCat) &&
                                                           SchrodingersCat.team == SchrodingersCat.Team.Jackal))
    ...
    private static void syncDummyPos()
    {
        // 前フレームからの経過時間をマイナスする
    ...
    public static class PlayerPhysicsPatch
    {
        public static void Postfix(PlayerPhysics __instance)
        {
            ...
            {
                PlayerControl puppeteer = __instance.myPlayer;
            ...
            {
                PlayerControl dummy = __instance.myPlayer;
    ...
    private class ExileControllerBeginPatch
    {
        ...
        {
            if (exiled != null && exiled.Object == target) OnTargetExiled();
        }
    }
        ...
        public static void Postfix(KeyboardJoystick __instance)
        {
            ...
            if (stealthed)
            {
                ...
                if (Input.GetKeyDown(KeyCode.E) || Input.GetKeyDown(KeyCode.Space))
                {
                    ...
                    foreach (PlainDoor door in doors)
                    {
                    ...
                    if (t != null)
                    {
                        DeconSystem deconSystem = t.transform.parent.gameObject.GetComponent<DeconSystem>();
                        if (deconSystem != null)
                        {
                            bool flag = true;
                            ...
                            Il2CppArrayBase<DeconControl> consoles = t.GetComponentsInChildren<DeconControl>();
                            DeconControl inner = null;
                            DeconControl outer = null;
                            foreach (DeconControl console in consoles)
                            {
                    ...
                    if (GameOptionsManager.Instance.currentNormalGameOptions.MapId == 4)
                    {
                        Ladder[] ladders = FastDestroyableSingleton<AirshipStatus>.Instance
                            .GetComponentsInChildren<Ladder>();
                        Ladder target = null;
                        ...
                        if (shipstatus != null)
                        {
                            List<PlatformConsole> consoles =
                                shipstatus.GetComponentsInChildren<PlatformConsole>().ToList();
                            ...
                            float distanceRight = Vector2.Distance(leftPlatform.transform.position,
                                dummy.transform.position);
                            float distanceLeft = Vector2.Distance(rightPlatform.transform.position,
                                dummy.transform.position);
                            if (distanceRight < 0.8f || distanceLeft < 0.8f)
                            {
                                ...
                                FastDestroyableSingleton<HudManager>.Instance
                                    .StartCoroutine(DontMove(1).WrapToIl2Cpp());
                    ...
                    {
                        IEnumerable elevators = Helpers.FindObjectsOfType(SubmarineElevatorType);
                        object elevator = null;
                        minDistance = 9999;
                        foreach (object e in elevators)
                        {
                            Vector3 pos = (e as MonoBehaviour).transform.position;
                            ...
                            PlainDoor lowerInnerDoor = lowerInnerDoorInfo.GetValue(e) as PlainDoor;
                            PlainDoor upperInnerDoor = upperInnerDoorInfo.GetValue(e) as PlainDoor;
                            float lowerDistance = Vector2.Distance(dummy.transform.position,
                                lowerInnerDoor.transform.position);
                            float upperDistance = Vector2.Distance(dummy.transform.position,
                                upperInnerDoor.transform.position);
                        ...
                        if (elevator != null)
                        {
                            MethodInfo use = SubmarineElevatorType.GetMethod("Use");
