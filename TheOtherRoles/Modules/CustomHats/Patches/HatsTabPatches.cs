...
internal static class HatsTabPatches
{
    ...
    private static bool OnEnablePrefix(HatsTab __instance)
    {
        ...
        IOrderedEnumerable<string> orderedKeys = packages.Keys.OrderBy(x =>
            x switch
            {
    ...
    private static float CreateHatPackage(List<Tuple<HatData, HatExtension>> hats, string packageName, float yStart,
        HatsTab hatsTab)
    {
        bool isDefaultPackage = CustomHatManager.InnerslothPackageName == packageName;
        if (!isDefaultPackage) hats = hats.OrderBy(x => x.Item1.name).ToList();

        float offset = yStart;
        if (textTemplate != null)
        {
            TextMeshPro title = Object.Instantiate(textTemplate, hatsTab.scroller.Inner);
            ...
            hatsTab.StartCoroutine(Effects.Lerp(0.1f,
                new Action<float>(p =>
                {
                    title.SetText(ModTranslation.getString(packageName.ToLower().Replace(" ", "")));
                })));
        ...
        for (int i = 0; i < hats.Count; i++)
        {
            ...
            if (ext != null)
            {
                ...
                if (textTemplate != null)
                {
                    TextMeshPro description = Object.Instantiate(textTemplate, colorChip.transform);
                    ...
                    hatsTab.StartCoroutine(Effects.Lerp(0.1f,
                        new Action<float>(p =>
                        {
                            description.SetText(string.Format(ModTranslation.getString("hatsAuthor"), hat.name,
                                ext.Author));
                        })));
                }
            }
