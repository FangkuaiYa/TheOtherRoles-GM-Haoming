...
public class HatsLoader : MonoBehaviour
{
    ...
    private IEnumerator CoFetchHats()
    {
        ...
        SkinsConfigFile response = JsonSerializer.Deserialize<SkinsConfigFile>(www.downloadHandler.text,
        ...
        List<string> toDownload = GenerateDownloadList(UnregisteredHats);

        TheOtherRolesPlugin.Logger.LogMessage($"I'll download {toDownload.Count} hat files");

        foreach (string fileName in toDownload) yield return CoDownloadHatAsset(fileName);
    ...
    private static IEnumerator CoDownloadHatAsset(string fileName)
    {
        ...
        string filePath = Path.Combine(HatsDirectory, fileName);
