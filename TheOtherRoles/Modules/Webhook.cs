...
internal class Webhook
{
    ...
    public static void post(List<Dictionary<string, object>> embeds, string bonusText, string extraText)
    {
        if (TheOtherRolesPlugin.WebhookUrl.Value != "" && bonusText != "forceEnd")
        {
            ...
            value.Add("content",
                "試合結果 " + ModTranslation.getString(bonusText) + " " + ModTranslation.getString(extraText));
            if (bonusText == "impostorWin")
                embeds[0].Add("color", 16711680);
            else if (bonusText == "crewWin") embeds[0].Add("color", 8375523);
            ...
            HttpResponseMessage response = client.PostAsync(TheOtherRolesPlugin.WebhookUrl.Value, content).Result;
        }
    }
}
