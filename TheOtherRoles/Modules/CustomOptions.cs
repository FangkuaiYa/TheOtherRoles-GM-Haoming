...
namespace TheOtherRoles
{
    ...
    public class CustomOption
    {
        ...
        public CustomOption(int id, CustomOptionType type, string name, System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader, bool isHidden, string format, Action onChange = null, string heading = "")
        {
            Init(id, type, name, selections, defaultValue, parent, isHeader, isHidden, format, onChange, heading);
        }

        public void Init(int id, CustomOptionType type, string name, System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader, bool isHidden, string format, Action onChange = null, string heading = "")
        {
            this.id = id;
            this.name = name;
            this.format = format;
        ...
        public static CustomOption Create(int id, CustomOptionType type, string name, string[] selections,
            CustomOption parent = null, bool isHeader = false, bool isHidden = false, string format = "", Action onChange = null, string heading = "")
        {
        return new CustomOption(id, type, name, selections, "", parent, isHeader, isHidden, format, onChange, heading);
        }


        public static CustomOption Create(int id, CustomOptionType type, string name, float defaultValue, float min, float max, float step, CustomOption parent = null, bool isHeader = false, bool isHidden = false, string format = "", Action onChange = null, string heading = "")
        {
            List<float> selections = new List<float>();
            for (float s = min; s <= max; s += step)
                selections.Add(s);
            return new CustomOption(id, type, name, selections.Cast<object>().ToArray(), defaultValue, parent, isHeader, isHidden, format, onChange, heading);
        }

        public static CustomOption Create(int id, CustomOptionType type, string name, bool defaultValue, CustomOption parent = null, bool isHeader = false, bool isHidden = false, string format = "", Action onChange = null, string heading = "")
        {
            return new CustomOption(id, type, name, new string[] { "optionOff", "optionOn" }, defaultValue ? "optionOn" : "optionOff", parent, isHeader, isHidden, format, onChange, heading);
        }
        ...
        {
            if (PlayerControl.AllPlayerControls.ToArray().ToList().Count <= 1 || AmongUsClient.Instance!.AmHost == false && PlayerControl.LocalPlayer == null) return;
        ...
        public static int pasteFromClipboard()
        {
            ...
            try
            {
                ...
                if (TheOtherRolesPlugin.Version > versionInfo && versionInfo < Version.Parse("2.3.137"))
                {
                    ...
                    FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(PlayerControl.LocalPlayer, "Host Info: Pasting vanilla settings failed, TORGMH Options applied!");
                }
                else
                {
    ...
    public class CustomRoleOption : CustomOption
    {
            ...
            base(id, type, Helpers.cs(color, name), CustomOptionHolder.rates, "", null, true, false, "")
        {
            ...
            if (max > 1)
                countOption = Create(id + 10000, type, "roleNumAssigned", 1f, 1f, 15f, 1f, this, false, isHidden, "unitPlayers", null, "");
        }
    ...
    class LobbyViewSettingsPatch
    {
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType)
        {
            ...
            foreach (var option in relevantOptions)
            {
                ...
                if (option == CustomOptionHolder.crewmateRolesCountMax || option == CustomOptionHolder.neutralRolesCountMax || option == CustomOptionHolder.impostorRolesCountMax)
                    continue;
                ...
                var settingTuple = handleSpecialOptionsView(option, option.getName(), ModTranslation.getString(option.selections[value].ToString()));
                ...
                if (option.isHeader && (int)optionType != 99 && option.getHeading() == "" && (option.type == CustomOptionType.Neutral || option.type == CustomOptionType.Crewmate || option.type == CustomOptionType.Impostor || option.type == CustomOptionType.Modifier))
                {
    ...
    class GameOptionsMenuStartPatch
    {
        ...
        private static void createSettings(GameOptionsMenu menu, List<CustomOption> options)
        {
            ...
            foreach (CustomOption option in options)
            {
                ...
                if (option.isHeader && option.getHeading() == "" && (option.type == CustomOptionType.Neutral || option.type == CustomOptionType.Crewmate || option.type == CustomOptionType.Impostor || option.type == CustomOptionType.Modifier))
                {
    ...
    class GameOptionsDataPatch
    {
        ...
        public static string buildModifierExtras(CustomOption customOption)
        {
            ...
            {
                return $" (1 " + ModTranslation.getString("buildModifierExtras") + $" {CustomOptionHolder.loversImpLoverRate.getSelection() * 10}%)";
            }
            return "";
        }
