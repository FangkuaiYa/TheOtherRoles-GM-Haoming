...
public static class Extensions
{
    ...
    private static readonly string[] ByteHex = (from x in Enumerable.Range(0, 256)
    ...
    {
        while (!self.IsCompleted) yield return null;
        yield break;
    }
    ...
    {
        for (int i = 0; i < self.Count; i++) Object.Destroy(self[i].gameObject);
        self.Clear();
    }
    ...
    {
        return string.Concat("<color=#", ByteHex[(byte)(c.r * 255f)], ByteHex[(byte)(c.g * 255f)],
            ByteHex[(byte)(c.b * 255f)], ByteHex[(byte)(c.a * 255f)], ">");
    }
    ...
    {
        if (alpha)
            return ((byte)(c.r * 256f) << 24) | ((byte)(c.g * 256f) << 16) | ((byte)(c.b * 256f) << 8) |
                   (byte)(c.a * 256f);
    ...
    public static KeyValuePair<byte, int> MaxPair(this Dictionary<byte, int> self, out bool tie)
    {
        ...
        foreach (KeyValuePair<byte, int> keyValuePair in self)
            ...
            else if (keyValuePair.Value == result.Value) tie = true;
    ...
    public static void AddAll<T>(this List<T> self, IList<T> other)
    {
        ...
        if (self.Capacity < num) self.Capacity = num;
        for (int i = 0; i < other.Count; i++) self.Add(other[i]);
    }
    ...
    public static void RandomFill<T>(this IList<T> self, T[] target)
    {
        ...
        for (int i = 0; i < target.Length; i++)
        {
            ...
            target[i] = self[num];
            hashSet.Add(num);
            if (hashSet.Count == self.Count) return;
        }
    }
    ...
    {
        for (int i = 0; i < self.Count; i++) todo(self[i]);
    }
    ...
    {
        enumValue = default;
        if (!Enum.IsDefined(typeof(TEnum), strEnumValue)) return false;
    ...
    {
        if (!Enum.IsDefined(typeof(TEnum), strEnumValue)) return default;
