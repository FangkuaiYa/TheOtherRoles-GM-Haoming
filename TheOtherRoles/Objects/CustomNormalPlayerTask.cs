...
public class CustomNormalPlayerTask
{
    ...
    private readonly NormalPlayerTask npt;

    public CustomNormalPlayerTask(string name, Type taskType, int maxStep, byte[] Data, SystemTypes startAt,
        bool showTaskStep)
    {
        Minigame task = new GameObject(name).AddComponent(taskType).TryCast<Minigame>();
        task.enabled = false;
        ...
        GameObject arrow = new("Arrow") { layer = 5 };
        ...
        npt.Arrow.image.color = Color.yellow;
    ...
    public void addTaskToPlayer(byte playerId)
    {
        ...
        NetworkedPlayerInfo.TaskInfo taskinfo = new((byte)npt.Id, npt.Id);
    ...
    {
        allTasks = new List<Shrine>();
        counter = 0;
        completedConsoles = new List<int>();
    }
}

[HarmonyPatch(typeof(NormalPlayerTask), nameof(NormalPlayerTask.UpdateArrowAndLocation))]
internal class UpdateArrowPatch
{
    public static bool Prefix(NormalPlayerTask __instance)
    {
        if (__instance.Id >= 100)
        {
            ...
            {
                if (__instance.name == "stay")
                {
                    if (Fox.taskType == Fox.TaskType.Serial)
                    {
                        __instance.Arrow.gameObject.SetActive(true);
                        Console console = ShipStatus.Instance.AllConsoles.FirstOrDefault(x =>
                            x.ConsoleId == __instance.Data[__instance.taskStep]);
...
internal class AppendTaskTextPatch
{
    public static bool Prefix(NormalPlayerTask __instance, [HarmonyArgument(0)] StringBuilder sb)
    {
...
internal class ValidConsolePatch
{
    public static bool Prefix(NormalPlayerTask __instance, [HarmonyArgument(0)] Console console, out bool __result)
    {
        if (__instance.TaskType == TaskTypes.None && console.TaskTypes.Contains(TaskTypes.None))
        {
            if (console.name == "shrine" && Fox.isStealthed(PlayerControl.LocalPlayer))
            {
                __result = false;
                return false;
            }
            ...
            if (__instance.MaxStep > 0)
            {
                if (console.name == "shrine" && Fox.taskType == Fox.TaskType.Parallel)
                {
                    if (!CustomNormalPlayerTask.completedConsoles.Contains(console.ConsoleId))
                    {
                        ...
                        for (int i = 0; i < __instance.MaxStep; i++) taskList.Add(__instance.Data[i]);
