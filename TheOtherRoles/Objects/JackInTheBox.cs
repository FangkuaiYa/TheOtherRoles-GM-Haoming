...
public class JackInTheBox
{
    ...
    public JackInTheBox(Vector2 p)
    {
        ...
        if (Helpers.isFungle())
        {
            ...
            SpriteAnim animator = vent.transform.GetChild(3).GetComponent<SpriteAnim>();
            animator?.Stop();
        }
        ...
        bool showBoxToLocalPlayer = PlayerControl.LocalPlayer == TheOtherRoles.Trickster.trickster ||
    ...
    public static Sprite getBoxAnimationSprite(int index)
    {
        ...
        if (boxAnimationSprites[index] == null)
            boxAnimationSprites[index] =
                TheOtherRolesPlugin.getResources($"TricksterAnimation/trickster_box_00{index + 1:00}.png");
    ...
    public static void UpdateStates()
    {
        ...
        foreach (JackInTheBox box in AllJackInTheBoxes)
        {
            bool showBoxToLocalPlayer = PlayerControl.LocalPlayer == TheOtherRoles.Trickster.trickster ||
                                        PlayerControl.LocalPlayer.Data.IsDead;
    ...
    {
        foreach (JackInTheBox box in AllJackInTheBoxes) box.convertToVent();
        connectVents();
    ...
    {
        for (int i = 0; i < AllJackInTheBoxes.Count - 1; i++)
        {
            JackInTheBox a = AllJackInTheBoxes[i];
            JackInTheBox b = AllJackInTheBoxes[i + 1];
            a.vent.Right = b.vent;
            b.vent.Left = a.vent;
        }
