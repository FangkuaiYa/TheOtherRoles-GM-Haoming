...
internal class HudManagerUpdatePatch
{
    ...
    private static void setNameColors()
    {
        if (PlayerControl.LocalPlayer.isRole(RoleType.Jester))
            ...
            setPlayerNameColor(PlayerControl.LocalPlayer,
                Swapper.swapper.Data.Role.IsImpostor ? Palette.ImpostorRed : Swapper.color);
        ...
        else if (PlayerControl.LocalPlayer.isRole(RoleType.Cupid) &&
                 PlayerControl.LocalPlayer.isAlive())
        {
            setPlayerNameColor(PlayerControl.LocalPlayer, Cupid.color);
            Cupid cupid = Cupid.allRoles.FirstOrDefault(x => x.player == PlayerControl.LocalPlayer) as Cupid;
        ...
        else if (PlayerControl.LocalPlayer.hasModifier(ModifierType.Munou) &&
                 PlayerControl.LocalPlayer.Data.IsDead)
            setPlayerNameColor(PlayerControl.LocalPlayer, Munou.color);

        else if (PlayerControl.LocalPlayer.hasModifier(ModifierType.AntiTeleport) &&
                 PlayerControl.LocalPlayer.Data.IsDead)
            setPlayerNameColor(PlayerControl.LocalPlayer, AntiTeleport.color);

        if (GM.gm != null) setPlayerNameColor(GM.gm, GM.color);
        ...
        if (Sidekick.sidekick != null && Sidekick.wasTeamRed &&
        ...
        if (PlayerControl.LocalPlayer.isRole(RoleType.Immoralist))
            foreach (PlayerControl fox in Fox.allPlayers)
                setPlayerNameColor(fox, Fox.color);

        PlayerControl player = PlayerControl.LocalPlayer;
        bool impostorFlag = player.isRole(RoleType.SchrodingersCat) || player.isImpostor();
        bool jackalFlag = player.isRole(RoleType.SchrodingersCat) || player.isRole(RoleType.Jackal) ||
        ...
        if (!SchrodingersCat.hasTeam() && SchrodingersCat.hideRole && PlayerControl.LocalPlayer.isAlive())
        {
            // 何もしない
        }
        else if (SchrodingersCat.team == SchrodingersCat.Team.Crew)
            foreach (PlayerControl p in SchrodingersCat.allPlayers)
        ...
        {
            foreach (PlayerControl p in SchrodingersCat.allPlayers) setPlayerNameColor(p, Jackal.color);
            if (player.isRole(RoleType.SchrodingersCat))
            {
        ...
        {
            foreach (PlayerControl p in SchrodingersCat.allPlayers) setPlayerNameColor(p, JekyllAndHyde.color);
            if (player.isRole(RoleType.SchrodingersCat))
                foreach (PlayerControl p in JekyllAndHyde.allPlayers)
                    setPlayerNameColor(p, JekyllAndHyde.color);
        }
        else if (SchrodingersCat.team == SchrodingersCat.Team.Moriarty && moriartyFlag)
        {
            foreach (PlayerControl p in SchrodingersCat.allPlayers) setPlayerNameColor(p, Moriarty.color);
            if (player.isRole(RoleType.SchrodingersCat))
                foreach (PlayerControl p in Moriarty.allPlayers)
                    setPlayerNameColor(p, Moriarty.color);
        }
        else if (player.isRole(RoleType.SchrodingersCat)) setPlayerNameColor(player, SchrodingersCat.color);
    ...
    private static void setNameTags()
    {
            ...
            foreach (PlayerControl player in PlayerControl.AllPlayerControls)
            {
                ...
                if (Godfather.godfather != null && Godfather.godfather == player)
                    player.cosmetics.nameText.text =
                        player.Data.PlayerName + $" ({ModTranslation.getString("mafiaG")})";
                else if (Mafioso.mafioso != null && Mafioso.mafioso == player)
                    player.cosmetics.nameText.text =
                        player.Data.PlayerName + $" ({ModTranslation.getString("mafiaM")})";
                else if (Janitor.janitor != null && Janitor.janitor == player)
                    player.cosmetics.nameText.text =
                        player.Data.PlayerName + $" ({ModTranslation.getString("mafiaJ")})";
            }

            if (MeetingHud.Instance != null)
                foreach (PlayerVoteArea player in MeetingHud.Instance.playerStates)
                    if (Godfather.godfather != null && Godfather.godfather.PlayerId == player.TargetPlayerId)
                        player.NameText.text = Godfather.godfather.Data.PlayerName +
                                               $" ({ModTranslation.getString("mafiaG")})";
                    else if (Mafioso.mafioso != null && Mafioso.mafioso.PlayerId == player.TargetPlayerId)
                        player.NameText.text =
                            Mafioso.mafioso.Data.PlayerName + $" ({ModTranslation.getString("mafiaM")})";
                    else if (Janitor.janitor != null && Janitor.janitor.PlayerId == player.TargetPlayerId)
                        player.NameText.text =
                            Janitor.janitor.Data.PlayerName + $" ({ModTranslation.getString("mafiaJ")})";
        }
        ...
        if (PlayerControl.LocalPlayer.isLovers() && PlayerControl.LocalPlayer.isAlive())
        {
            ...
            if (Cupid.isCupidLovers(PlayerControl.LocalPlayer)) suffix = Helpers.cs(Cupid.color, " ♥");
            PlayerControl lover1 = PlayerControl.LocalPlayer;
            PlayerControl lover2 = PlayerControl.LocalPlayer.getPartner();
        ...
        if (PlayerControl.LocalPlayer.isAlive() && PlayerControl.LocalPlayer.isAkujoLover())
            foreach (Akujo akujo in Akujo.players)
            {
                ...
                if (PlayerControl.LocalPlayer == akujo.player)
                {
                    ...
                    foreach (AkujoKeep keep in akujo.keeps)
                ...
                else if (PlayerControl.LocalPlayer == akujo.honmei?.player ||
                         akujo.keeps.Any(x => PlayerControl.LocalPlayer == x?.player))
                {
                    PlayerControl.LocalPlayer.cosmetics.nameText.text += suffix;
                    ...
                    if (Helpers.ShowMeetingText)
                        foreach (PlayerVoteArea player in MeetingHud.Instance.playerStates)
                            if (PlayerControl.LocalPlayer.PlayerId == player.TargetPlayerId ||
                                akujo.player.PlayerId == player.TargetPlayerId)
                                player.NameText.text += suffix;
                }
            }

        if (TORMapOptions.ghostsSeeRoles && PlayerControl.LocalPlayer.isDead())
        {
            foreach (Couple couple in Lovers.couples)
            {
                string suffix = Lovers.getIcon(couple.lover1);
                if (Cupid.isCupidLovers(couple.lover1)) suffix = Helpers.cs(Cupid.color, " ♥");
                couple.lover1.cosmetics.nameText.text += suffix;
                couple.lover2.cosmetics.nameText.text += suffix;

                if (meetingShow)
                    foreach (PlayerVoteArea player in MeetingHud.Instance.playerStates)
                        if (couple.lover1.PlayerId == player.TargetPlayerId ||
                            couple.lover2.PlayerId == player.TargetPlayerId)
                            player.NameText.text += suffix;
            }
        ...
        // Lawyer
        bool localIsLawyer = Lawyer.lawyer != null && Lawyer.target != null &&
                             Lawyer.lawyer == PlayerControl.LocalPlayer;
        bool localIsKnowingTarget = Lawyer.lawyer != null && Lawyer.target != null && Lawyer.targetKnows &&
                                    Lawyer.target == PlayerControl.LocalPlayer;
        ...
        if (PlayerControl.LocalPlayer != null && TORMapOptions.showLighterDarker)
            if (meetingShow)
                foreach (PlayerVoteArea player in MeetingHud.Instance.playerStates)
                {
                    PlayerControl target = Helpers.playerById(player.TargetPlayerId);
                    if (target != null)
                        player.NameText.text +=
                            $" ({(Helpers.isLighterColor(target.Data.DefaultOutfit.ColorId) ? ModTranslation.getString("detectiveLightLabel") : ModTranslation.getString("detectiveDarkLabel"))})";
                }
    }
    ...
    {
        foreach (Mini mini in Mini.players) _miniUpdate(mini);
    }
    ...
    public static void _miniUpdate(Mini mini)
    {
        ...
        if (Morphling.morphling != null && Morphling.morphTarget == mini.player && Morphling.morphTimer > 0f &&
            !Helpers.hidePlayerName(Morphling.morphling))
    ...
    private static void camouflageAndMorphActions()
    {
        ...
        // Morphling reset
        if (oldMorphTimer > 0f && Morphling.morphTimer <= 0f) Morphling.resetMorph();
    }
