...
internal class OptimizeMapPatch
{
    ...
    {
        addWireTasks(GameOptionsManager.Instance.currentNormalGameOptions.MapId);
    ...
    public static void optimizeMap(int mapId)
    {
        ...
        if (mapId == 4)
        {
            ...
            EdgeCollider2D Collider = fance.AddComponent<EdgeCollider2D>();
            Collider.points = new[] { new Vector2(1.5f, -0.2f), new Vector2(-1.5f, -0.2f), new Vector2(-1.5f, 1.5f) };
    ...
    public static void addLadder(int mapId)
    {
        if (mapId == 4)
        {
            ...
            {
                // 梯子追加
                GameObject ladder = meetingRoom.GetComponentsInChildren<SpriteRenderer>()
                    .Where(x => x.name == "ladder_meeting").FirstOrDefault().gameObject;
                ...
                if (!ladderSprite)
                    ladderSprite = Helpers.loadSpriteFromResources("TheOtherRoles.Resources.ladder.png", 100f);
                newLadder.GetComponentInChildren<SpriteRenderer>().sprite = ladderSprite;

                // 梯子の周りの影を消す
                GameObject.Destroy(gapRoom.GetComponentsInChildren<EdgeCollider2D>()
                    .Where(x => Math.Abs(x.points[0].x + 6.2984f) < 0.1).FirstOrDefault());
                EdgeCollider2D collider = meetingRoom.GetComponentsInChildren<EdgeCollider2D>()
                    .Where(x => x.pointCount == 46).FirstOrDefault();
                ...
                points.Clear();
                foreach (int i in Enumerable.Range(0, 41)) points.Add(collider.points[i]);
                newCollider2.SetPoints(points);
                GameObject.DestroyObject(collider);

                // 梯子の背景を変更
                SpriteRenderer side = meetingRoom.GetComponentsInChildren<SpriteRenderer>()
                    .Where(x => x.name == "meeting_side").FirstOrDefault();
                SpriteRenderer bg = GameObject.Instantiate(side, side.transform.parent);
                if (!ladderBgSprite)
                    ladderBgSprite = Helpers.loadSpriteFromResources("TheOtherRoles.Resources.ladder_bg.png", 100f);
            ...
            if (CustomOptionHolder.airshipOneWayLadder.getBool())
            {
                GameObject ladder = meetingRoom.GetComponentsInChildren<SpriteRenderer>()
                    .Where(x => x.name == "ladder_meeting").FirstOrDefault().gameObject;
                ladder.GetComponentsInChildren<Ladder>().Where(x => x.name == "LadderTop").FirstOrDefault().gameObject
                    .SetActive(false);
            }
        }
    }
...
internal class NormalPlayerTaskPickRandomConsolesPatch
{
    ...
    private static void Postfix(NormalPlayerTask __instance, TaskTypes taskType, byte[] consoleIds)
    {
        if (taskType != TaskTypes.FixWiring || !CustomOptionHolder.randomWireTask.getBool()) return;
        System.Collections.Generic.List<Console> orgList =
            ShipStatus.Instance.AllConsoles.Where(t => t.TaskTypes.Contains(taskType)).ToList();
        System.Collections.Generic.List<Console> list = new(orgList);
        ...
        {
            if (list.Count == 0)
                list = new System.Collections.Generic.List<Console>(orgList);
