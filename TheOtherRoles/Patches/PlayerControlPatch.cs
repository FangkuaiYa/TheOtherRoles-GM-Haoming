...
{
    private static readonly Dictionary<byte, bool> lastVisible = new();
    // Helpers

    public static PlayerControl setTarget(bool onlyCrewmates = false, bool targetPlayersInVents = false,
        List<PlayerControl> untargetablePlayers = null, PlayerControl targetingPlayer = null, int killDistance = 3)
    {
        ...
        if ((targetingPlayer.Data.IsDead && !targetingPlayer.isRole(RoleType.Puppeteer)) ||
            targetingPlayer.inVent) return result;
        ...
        if (Puppeteer.exists)
        {
            ...
            {
                PlayerControl puppeteer = Puppeteer.players.FirstOrDefault().player;
                untargetablePlayers.Add(puppeteer);
            }
            else if (!Puppeteer.stealthed && Puppeteer.dummy != null) untargetablePlayers.Add(Puppeteer.dummy);
        }
    ...
    private static void setBasePlayerOutlines()
    {
        foreach (PlayerControl target in PlayerControl.AllPlayerControls)
        {
            ...
            bool isMorphedMorphling = target == Morphling.morphling && Morphling.morphTarget != null &&
            ...
            if (Camouflager.camouflageTimer <= 0f && Medic.shielded != null &&
                ...
                hasVisibleShield = Medic.showShielded == 0 // Everyone
                                   || (Medic.showShielded == 1 &&
                                       (PlayerControl.LocalPlayer == Medic.shielded ||
                                        PlayerControl.LocalPlayer == Medic.medic)) // Shielded + Medic
    ...
    public static void bendTimeUpdate()
    {
        if (TimeMaster.isRewinding)
        {
            ...
            {
                // Set position
                Tuple<Vector3, bool> next = localPlayerPositions[0];
                if (next.Item2)
                {
                    ...
                    if (PlayerControl.LocalPlayer.inVent)
                        ...
                        {
                            vent.CanUse(PlayerControl.LocalPlayer.Data, out bool canUse,
                                out bool couldUse);
                            if (canUse)
                            {
    ...
    private static void shifterUpdate()
    {
        ...
        if (Shifter.isNeutral && !Shifter.shiftPastShifters)
        {
            ...
            foreach (int playerId in Shifter.pastShifters) blockShift.Add(Helpers.playerById((byte)playerId));
        }
    ...
    private static void evilHackerSetTarget()
    {
    ...
    private static void sidekickCheckPromotion()
    {
            ...
            (Jackal.jackal == null || Jackal.jackal.Data.Disconnected))
        {
            MessageWriter writer = AmongUsClient.Instance.StartRpcImmediately(
                PlayerControl.LocalPlayer.NetId, (byte)CustomRPC.SidekickPromotes, SendOption.Reliable);
    ...
    private static void engineerUpdate()
    {
        ...
        bool jackalHighlight = Engineer.highlightForTeamJackal &&
                               (PlayerControl.LocalPlayer == Jackal.jackal ||
                                PlayerControl.LocalPlayer == Sidekick.sidekick);
        bool impostorHighlight = Engineer.highlightForImpostors &&
    ...
    private static void impostorSetTarget()
    {
        if (!PlayerControl.LocalPlayer.Data.Role.IsImpostor ||
            !PlayerControl.LocalPlayer.CanMove || PlayerControl.LocalPlayer.Data.IsDead)
        {
    ...
    private static void assassinUpdate()
    {
        if (Assassin.arrow?.arrow != null)
        {
            if (Assassin.assassin == null || Assassin.assassin != PlayerControl.LocalPlayer ||
                !Assassin.knowsTargetLocation)
            {
            ...
            if (Assassin.assassinMarked != null && !PlayerControl.LocalPlayer.Data.IsDead)
            {
                ...
                if (!trackedOnMap)
                {
                    ...
                    DeadBody body = Object.FindObjectsOfType<DeadBody>()
                        .FirstOrDefault(b => b.ParentId == Assassin.assassinMarked.PlayerId);
                    if (body != null)
                    {
                        trackedOnMap = true;
    ...
    private static void trackerUpdate()
    {
        ...
        if (Tracker.arrow?.arrow != null)
        {
            ...
            if (Tracker.tracker != null && Tracker.tracked != null &&
                PlayerControl.LocalPlayer == Tracker.tracker && !Tracker.tracker.Data.IsDead)
            {
    ...
    public static void playerSizeUpdate(PlayerControl p)
    {
        ...
        foreach (Mini mini in Mini.players)
        {
            ...
            if (Morphling.morphling != null && p == Morphling.morphling &&
                Morphling.morphTarget.hasModifier(ModifierType.Mini) && Morphling.morphTimer > 0f)
            {
                p.transform.localScale = new Vector3(scale, scale, 1f);
                collider.radius = correctedColliderRadius;
            }
    ...
    public static void updatePlayerInfo()
    {
        ...
        foreach (PlayerControl p in PlayerControl.AllPlayerControls)
        {
            ...
            bool canSeeInfo =
                ...
                (Lawyer.lawyerKnowsRole && PlayerControl.LocalPlayer == Lawyer.lawyer &&
                 p == Lawyer.target);
            ...
            if (canSeeInfo)
            {
                ...
                playerInfo.transform.localPosition =
                    p.cosmetics.nameText.transform.localPosition + (Vector3.up * 0.225f);
                ...
                Transform meetingInfoTransform = playerVoteArea != null
                    ...
                    : null;
                TextMeshPro meetingInfo = meetingInfoTransform != null
                ...
                (int tasksCompleted, int tasksTotal) = TasksHandler.taskInfo(p.Data);
                string roleNames = RoleInfo.GetRolesString(p, true, new[] { RoleType.Lovers });
                string roleNamesFull = RoleInfo.GetRolesString(p, true, new[] { RoleType.Lovers }, true);


                string completedStr = commsActive ? "?" : tasksCompleted.ToString();
                ...
                if (p == PlayerControl.LocalPlayer)
                {
                    ...
                    {
                        TextMeshPro tabText = FastDestroyableSingleton<TaskPanelBehaviour>.Instance.tab.transform
                ...
                else if (TORMapOptions.ghostsSeeRoles && TORMapOptions.ghostsSeeTasks)
                {
                    playerInfoText = $"{roleNames} {taskInfo}".Trim();
                    meetingInfoText = playerInfoText;
                }
                else if (TORMapOptions.ghostsSeeTasks)
                {
    ...
    public static void securityGuardUpdate()
    {
        ...
        (int playerCompleted, _) = TasksHandler.taskInfo(SecurityGuard.securityGuard.Data);
    ...
    private static void bountyHunterUpdate()
    {
        if (BountyHunter.bountyHunter == null ||
            PlayerControl.LocalPlayer != BountyHunter.bountyHunter) return;
        ...
        if (BountyHunter.bounty == null || BountyHunter.bountyUpdateTimer <= 0f)
        {
                ...
                FastDestroyableSingleton<HudManager>.Instance.UseButton != null)
            {
                    ...
                    TORMapOptions.playerIcons[BountyHunter.bounty.PlayerId].gameObject != null)
                {
                    ...
                    TORMapOptions.playerIcons[BountyHunter.bounty.PlayerId].transform.localScale = Vector3.one * 0.4f;
                }
            }
        }
    ...
    private static void assassinSetTarget()
    {
        ...
        foreach (Mini mini in Mini.players) untargetables.Add(mini.player);
        ...
        Assassin.currentTarget = setTarget(true, untargetablePlayers: untargetables);
    ...
    private static void baitUpdate()
    {
        ...
        if (MapUtilities.CachedShipStatus?.AllVents != null)
        {
            ...
            foreach (PlayerControl player in PlayerControl.AllPlayerControls)
            {
                ...
                if (player.inVent)
                {
                    Vent target = MapUtilities.CachedShipStatus.AllVents
    ...
    {
        if (Medium.medium == null || Medium.medium != PlayerControl.LocalPlayer ||
    ...
    private static void gmUpdate()
    {
        ...
        {
            HudManager.Instance.TaskPanel.enabled = false;
            HudManager.Instance.TaskPanel.background.enabled = false;
            HudManager.Instance.TaskPanel.tab.enabled = false;
            HudManager.Instance.TaskPanel.taskText.enabled = false;
            HudManager.Instance.TaskPanel.tab.transform.FindChild("TabText_TMP").GetComponent<TextMeshPro>()
                .SetText("");
            //HudManager.Instance.TaskPanel.transform.localPosition = Vector3.negativeInfinityVector;
        }
    }
    ...
    public static void lawyerUpdate()
    {
        ...
        // Meeting win
        if (Lawyer.winsAfterMeetings && Lawyer.neededMeetings == Lawyer.meetings && Lawyer.target != null &&
            !Lawyer.target.Data.IsDead)
        {
            Lawyer.winsAfterMeetings = false; // Avoid sending mutliple RPCs until the host finshes the game
            MessageWriter winWriter = AmongUsClient.Instance.StartRpcImmediately(PlayerControl.LocalPlayer.NetId,
                (byte)CustomRPC.LawyerWin, SendOption.Reliable);
    ...
    public static void hackerUpdate()
    {
        ...
        (int playerCompleted, _) = TasksHandler.taskInfo(Hacker.hacker.Data);
    ...
    public static void Postfix(PlayerControl __instance)
    {
        if (lastVisible.TryGetValue(__instance.PlayerId, out bool visible) && __instance.Visible != visible)
            LogHelper.Info($"Visible Status Change to {__instance.Visible} for {__instance.getNameWithRole()}",
                "BodySprite");
...
internal class PlayerPhysicsWalkPlayerToPatch
{
    ...
    {
        bool correctOffset = Camouflager.camouflageTimer <= 0f && (__instance.myPlayer.hasModifier(ModifierType.Mini) ||
                                                                   (Morphling.morphling != null &&
                                                                    __instance.myPlayer == Morphling.morphling &&
                                                                    Morphling.morphTarget
                                                                        .hasModifier(ModifierType.Mini) &&
                                                                    Morphling.morphTimer > 0f));
        if (correctOffset)
        {
...
{
    private static bool Prefix(PlayerControl __instance, [HarmonyArgument(0)] NetworkedPlayerInfo target)
    {
        if (Moriarty.brainwashed.FindAll(x => x.PlayerId == __instance.PlayerId).Count > 0) return false;
        return true;
    }

    private static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] NetworkedPlayerInfo target)
    {
        LogHelper.Info($"{__instance.getNameWithRole()} => {target?.getNameWithRole() ?? "null"}", "ReportDeadBody");
        // Medic or Detective report
        bool isMedicReport = Medic.medic != null && Medic.medic == PlayerControl.LocalPlayer &&
                             __instance.PlayerId == Medic.medic.PlayerId;
        bool isDetectiveReport = Detective.detective != null &&
            ...
            if (deadPlayer != null && deadPlayer.killerIfExisting != null)
            {
                ...
                else if (isDetectiveReport)
                {
                    if (timeSinceDeath < Detective.reportNameDuration * 1000)
                        msg = string.Format(ModTranslation.getString("detectiveReportName"),
                            deadPlayer.killerIfExisting.Data.PlayerName);
...
public static class MurderPlayerPatch
{
    ...
    public static void Prefix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target)
    {
        ...
        if (Morphling.morphling != null && target == Morphling.morphling) Morphling.resetMorph();
    ...
    {
        LogHelper.Info($"{__instance.getNameWithRole()} => {target.getNameWithRole()}", "MurderPlayer");
        ...
        if (Assassin.assassin != null && PlayerControl.LocalPlayer == Assassin.assassin &&
            __instance == Assassin.assassin && HudManagerStartPatch.assassinButton != null)
        ...
        if (Seer.seer != null && PlayerControl.LocalPlayer == Seer.seer && !Seer.seer.Data.IsDead &&
            Seer.seer != target && Seer.mode <= 1) Helpers.showFlash(new Color(42f / 255f, 187f / 255f, 245f / 255f));
        ...
        if (PlayerControl.LocalPlayer.hasModifier(ModifierType.Mini) &&
            PlayerControl.LocalPlayer.Data.Role.IsImpostor &&
        ...
        if (Bait.bait != null && target == Bait.bait && Bait.showKillFlash && __instance != Bait.bait &&
        ...
        if (target.isImpostor() && AmongUsClient.Instance.AmHost) LastImpostor.promoteToLastImpostor();

        // 人形使いのダミー死亡処理
        if (target == Puppeteer.dummy)
        {
            Helpers.checkMuderAttempt(__instance, target);
            // 死体を消す
...
public static class AssignRolePatch
{
    ...
    {
        SchrodingersCat.AssignRoleOnDeath(player);
        Puppeteer.AssignRoleOnDeath(player); // 看是不是伪人[doge]是伪人就给职业[doge]
        return true;
    }
}
...
internal static class PlayerControlSetCoolDownPatch
{
    public static bool Prefix(PlayerControl __instance, [HarmonyArgument(0)] float time)
    {
        ...
        if (PlayerControl.LocalPlayer.hasModifier(ModifierType.Mini) &&
            ...
            multiplier = Mini.isGrownUp(PlayerControl.LocalPlayer) ? 0.66f : 2f;
        if (PlayerControl.LocalPlayer.isRole(RoleType.BountyHunter))
            addition = BountyHunter.punishmentTime;
        if (PlayerControl.LocalPlayer.isRole(RoleType.Ninja) &&
            Ninja.isPenalized(PlayerControl.LocalPlayer)) addition = Ninja.killPenalty;
...
public static class ExilePlayerPatch
{
    public static void Postfix(PlayerControl __instance)
    {
        ...
        // impostor promote to last impostor
        if (__instance.isImpostor() && AmongUsClient.Instance.AmHost) LastImpostor.promoteToLastImpostor();
    }
}
...
internal class PlayerControlCheckNamePatch
{
    public static bool Prefix(PlayerControl __instance, [HarmonyArgument(0)] string name)
    {
        if (CustomOptionHolder.uselessOptions.getBool() && CustomOptionHolder.playerNameDupes.getBool())
        {
            ...
            GameData.Instance.UpdateName(__instance.PlayerId, name);
            return false;
        }
