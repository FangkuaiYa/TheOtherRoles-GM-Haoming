...
namespace TheOtherRoles.Patches
{
    ...
    public static class PlayerControlFixedUpdatePatch
    {
        ...
        public static void playerSizeUpdate(PlayerControl p)
        {
            ...
            foreach (var mini in Mini.allPlayers)
            {
                // Set adapted player size to Mini and Morphling
                if (Camouflager.camouflageTimer > 0f || Helpers.MushroomSabotageActive() || (mini == Morphling.morphling && Morphling.morphTimer > 0) || (mini.isRole(RoleType.MimicA) && MimicA.isMorph)) return;
            }
        ...
        public static void arsonistSetTarget()
        {
            ...
            if (Arsonist.douseTarget != null)
                untargetables = PlayerControl.AllPlayerControls.ToArray().ToArray().Where(x => x.PlayerId != Arsonist.douseTarget.PlayerId).ToList();
            else
        ...
        static void baitUpdate()
        {
            ...
            if (Bait.bait.Data.IsDead && !Bait.reported)
            {
                ...
                if (deadPlayer.killerIfExisting != null && Bait.reportDelay <= 0f)
                {
                    ...
                    {
                        var candidates = PlayerControl.AllPlayerControls.ToArray().ToArray().Where(x => x.isAlive() && !x.isImpostor() && !x.isDummy).ToList();
        ...
        static void gmUpdate()
        {
            ...
            if (HudManager.Instance.TaskPanel != null)
            {
                ...
                HudManager.Instance.TaskPanel.tab.transform.FindChild("TabText_TMP").GetComponent<TMPro.TextMeshPro>().SetText("");
                //THudManager.Instance.TaskPanel.transform.localPosition = Vector3.negativeInfinityVector;
            }
        ...
        static void witchSetTarget()
        {
            ...
            if (Witch.spellCastingTarget != null)
                untargetables = PlayerControl.AllPlayerControls.ToArray().ToArray().Where(x => x.PlayerId != Witch.spellCastingTarget.PlayerId).ToList(); // Don't switch the target from the the one you're currently casting a spell on
            else
            {
            ...
            if (isMedicReport || isDetectiveReport)
            {
                ...
                if (deadPlayer != null && deadPlayer.killerIfExisting != null)
                {
                    ...
                    else if (isDetectiveReport)
                    {
                        ...
                        {
                            var typeOfColor = Helpers.isLighterColor(deadPlayer.killerIfExisting) ?
    ...
    static class PlayerControlSetCoolDownPatch
    {
        public static bool Prefix(PlayerControl __instance, [HarmonyArgument(0)] float time)
        {
            ...
            if (GameOptionsManager.Instance.currentNormalGameOptions.KillCooldown <= 0f) return false; float multiplier = 1f;
            float addition = 0f;
    ...
    class PlayerControlCanMovePatch
    {
        public static bool Prefix(PlayerControl __instance, ref bool __result)
        {
            __result = __instance.moveable &&
                !Minigame.Instance &&
                (!DestroyableSingleton<HudManager>.InstanceExists || (FastDestroyableSingleton<HudManager>.Instance.Chat.IsClosedOrClosing && !FastDestroyableSingleton<HudManager>.Instance.KillOverlay.IsOpen && !FastDestroyableSingleton<HudManager>.Instance.GameMenu.IsOpen)) &&
    ...
    class PlayerControlCheckNamePatch
    {
        public static bool Prefix(PlayerControl __instance, [HarmonyArgument(0)] string name)
        {
            if (CustomOptionHolder.uselessOptions.getBool() && CustomOptionHolder.playerNameDupes.getBool())
            {
                ...
                __instance.Data.UpdateNamePlate(name);
                return false;
            }
