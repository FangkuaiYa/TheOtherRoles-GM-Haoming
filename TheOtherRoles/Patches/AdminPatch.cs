...
{
    private static Dictionary<SystemTypes, List<Color>> playerColors = new();
    private static float adminTimer;
    private static TextMeshPro OutOfTime;
    private static TextMeshPro TimeRemaining;
    private static bool clearedIcons;
    public static bool isEvilHackerAdmin;
    private static Sprite adminCockpitSprite;
    private static Sprite adminRecordsSprite;
    private static GameObject map;
    private static GameObject newmap;

    private static PlainShipRoom room;

    private static readonly List<SystemTypes> filterCockpitAdmin = new()
    {
        SystemTypes.Cockpit,
        SystemTypes.Armory,
        SystemTypes.Kitchen,
    ...
    private static readonly List<SystemTypes> filterRecordsAdmin = new()
    {
        SystemTypes.Records,
        SystemTypes.Lounge,
    ...
    private static bool filterAdmin(SystemTypes type)
    {
        ...
        {
            if (room.name == "Cockpit" && !filterCockpitAdmin.Contains(type)) return false;
            if (room.name == "Records" && !filterRecordsAdmin.Contains(type)) return false;
        }
    ...
    {
        public static bool Prefix(ref float __result, MapConsole __instance,
    ...
    private class MapCountOverlayOnEnablePatch
    {
        private static void Prefix(MapCountOverlay __instance)
        {
                ...
                (room.name == "Cockpit" || room.name == "Records"))
            {
                if (room.name == "Cockpit" && !adminCockpitSprite)
                    adminCockpitSprite =
                        Helpers.loadSpriteFromResources("TheOtherRoles.Resources.admin_cockpit.png", 100f);
                if (room.name == "Records" && !adminRecordsSprite)
                    adminRecordsSprite =
                        Helpers.loadSpriteFromResources("TheOtherRoles.Resources.admin_records.png", 100f);
                if (!map)
                    map = DestroyableSingleton<MapBehaviour>.Instance.gameObject
                        .GetComponentsInChildren<SpriteRenderer>().Where(x => x.name == "Background").FirstOrDefault()
                        .gameObject;
                if (!newmap) newmap = Object.Instantiate(map, map.transform.parent);

                SpriteRenderer renderer = newmap.GetComponent<SpriteRenderer>();
                if (room.name == "Cockpit")
                {
                    renderer.sprite = adminCockpitSprite;
                    newmap.transform.position = new Vector3(map.transform.position.x + 0.5f, map.transform.position.y,
                        map.transform.position.z - 0.1f);
                }

                if (room.name == "Records")
                {
                    renderer.sprite = adminRecordsSprite;
                    newmap.transform.position = new Vector3(map.transform.position.x - 0.38f, map.transform.position.y,
                        map.transform.position.z - 0.1f);
                }
    ...
    private class MapCountOverlayUpdatePatch
    {
        private static bool Prefix(MapCountOverlay __instance)
        {
            ...
            if (TORMapOptions.restrictDevices > 0 && TORMapOptions.restrictAdmin)
            {
                ...
                {
                    if (TORMapOptions.restrictAdminTime <= 0f && !PlayerControl.LocalPlayer.isImpostor())
                    {
                    ...
                    //TimeRemaining.color = TORMapOptions.restrictAdminTime > 10f ? Palette.AcceptedGreen : Palette.ImpostorRed;
            ...
            if (CustomOptionHolder.impostorCanIgnoreComms.getBool() &&
                PlayerControl.LocalPlayer.isImpostor()) commsActive = false;
            ...
            for (int i = 0; i < __instance.CountAreas.Length; i++)
            {
                ...
                if (!commsActive && counterArea.RoomType > SystemTypes.Hallway)
                {
                    ...
                    if (plainShipRoom != null && plainShipRoom.roomArea)
                    {
                        ...
                        if (CustomOptionHolder.airshipOldAdmin.getBool() &&
                            (counterArea.RoomType == SystemTypes.Ventilation ||
                             counterArea.RoomType == SystemTypes.HallOfPortraits)) num2 = 0;

                        // アドミン毎に表示する範囲を制限する
                        if (!filterAdmin(counterArea.RoomType)) num2 = 0;
            ...
            if (playerColors.ContainsKey(__instance.RoomType))
            {
                ...
                foreach (PlayerControl p in PlayerControl.AllPlayerControls)
                {
                    ...
                    if (p.isImpostor())
                    {
                        ...
                        if (p.isRole(RoleType.MimicK)) mimicKColors.Add(color);
                    }
                ...
                for (int i = 0; i < __instance.myIcons.Count; i++)
                {
                    ...
                    if (renderer != null)
                    {
                        ...
                        else if (((PlayerControl.LocalPlayer.isRole(RoleType.EvilHacker) ||
                                   EvilHacker.isInherited()) && EvilHacker.canHasBetterAdmin) ||
                                 PlayerControl.LocalPlayer.isRole(RoleType.MimicA))
                        {
                            renderer.material = newMat;
                            Color color = colors[i];
