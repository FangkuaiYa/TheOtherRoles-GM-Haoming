...
public static class UsablesPatch
{
    public static bool IsBlocked(PlayerTask task, PlayerControl pc)
    {
        ...
        if (pc.isRole(RoleType.Swapper) && (isLights || isComms)) return true;

        if (pc.hasModifier(ModifierType.Madmate) && (isLights || (isComms && !Madmate.canFixComm))) return true;

        if (pc.hasModifier(ModifierType.CreatedMadmate) &&
            (isLights || (isComms && !CreatedMadmate.canFixComm))) return true;

        if (pc.isGM() && (isLights || isComms || isReactor || isO2)) return true;

        if (pc.isRole(RoleType.Mafioso) && !Mafioso.canRepair && (isLights || isComms)) return true;

        if (pc.isRole(RoleType.Janitor) && !Janitor.canRepair && (isLights || isComms)) return true;

        if (pc.isRole(RoleType.Fox) && (isLights || isComms || isReactor || isO2))
        {
            if (isReactor) return true;
    ...
    public static bool IsBlocked(SystemConsole console, PlayerControl pc)
    {
        ...
        if ((isSecurity && !canUseCameras) || (isVitals && !canUseVitals)) return true;
        return false;
    }
    ...
    public static bool IsBlocked(IUsable target, PlayerControl pc)
    {
        ...
        if (PlayerControl.LocalPlayer.isRole(RoleType.JekyllAndHyde) && !JekyllAndHyde.isJekyll())
        {
            ...
            bool isComms = task?.TaskType == TaskTypes.FixComms;
            bool isReactor =
                task?.TaskType is TaskTypes.StopCharles or TaskTypes.ResetSeismic or TaskTypes.ResetReactor;
            bool isO2 = task?.TaskType == TaskTypes.RestoreOxy;
            if (!isSecurity || !isVitals || !isButton || !isLights || !isComms || !isReactor || !isO2) return true;
        }

        if ((targetConsole != null && IsBlocked(targetConsole, pc)) ||
            (targetSysConsole != null && IsBlocked(targetSysConsole, pc)) ||
            (targetMapConsole != null && !canUseAdmin))
            return true;
        return false;
    }
    ...
    private class VentButtonDoClickPatch
    {
    ...
    public static class VentUsePatch
    {
        public static bool Prefix(Vent __instance)
        {
            ...
            bool canMoveInVents = PlayerControl.LocalPlayer != Spy.spy &&
                                  !PlayerControl.LocalPlayer.hasModifier(ModifierType.Madmate) &&
                                  !PlayerControl.LocalPlayer.hasModifier(ModifierType.CreatedMadmate);
    ...
    private class VentButtonVisibilityPatch
    {
    ...
    private class VentButtonSetTargetPatch
    {
        ...
        private static void Postfix(VentButton __instance)
        {
            ...
            if (Trickster.trickster != null && Trickster.trickster == PlayerControl.LocalPlayer)
            {
                ...
                bool isSpecialVent = __instance.currentTarget != null && __instance.currentTarget.gameObject != null &&
                                     __instance.currentTarget.gameObject.name.StartsWith("JackInTheBoxVent_");
                __instance.graphic.sprite =
                    isSpecialVent ? Trickster.getTricksterVentButtonSprite() : defaultVentSprite;
    ...
    private class KillButtonDoClickPatch
    {
        public static bool Prefix(KillButton __instance)
        {
                ...
                PlayerControl.LocalPlayer.isAlive() && PlayerControl.LocalPlayer.CanMove)
            {
                ...
                if (PlayerControl.LocalPlayer.isRole(RoleType.Ninja) &&
                    Ninja.isStealthed(PlayerControl.LocalPlayer)) showAnimation = false;

                // Use an unchecked kill command, to allow shorter kill cooldowns etc. without getting kicked
                MurderAttemptResult res = Helpers.checkMuderAttemptAndKill(PlayerControl.LocalPlayer,
                    __instance.currentTarget, showAnimation: showAnimation);
                // Handle blank kill
                if (res == MurderAttemptResult.BlankKill)
                {
                    ...
                    if (PlayerControl.LocalPlayer == Cleaner.cleaner)
                        ...
                        Assassin.assassin.killTimer = HudManagerStartPatch.assassinButton.Timer =
                            HudManagerStartPatch.assassinButton.MaxTimer;
                }
    ...
    private class SabotageButtonRefreshPatch
    {
        private static void Postfix()
        {
            // Mafia disable sabotage button for Janitor and sometimes for Mafioso
            bool blockSabotageJanitor =
                Janitor.janitor != null && Janitor.janitor == PlayerControl.LocalPlayer;
            bool blockSabotageMafioso = Mafioso.mafioso != null &&
                                        Mafioso.mafioso == PlayerControl.LocalPlayer &&
    ...
    private class UseButtonSetTargetPatch
    {
        private static bool Prefix(UseButton __instance, [HarmonyArgument(0)] IUsable target)
        {
    ...
    private class EmergencyMinigameUpdatePatch
    {
        private static void Postfix(EmergencyMinigame __instance)
        {
            ...
            if (Lawyer.lawyer != null && Lawyer.lawyer == PlayerControl.LocalPlayer &&
                Lawyer.winsAfterMeetings)
            {
                roleCanCallEmergency = false;
                statusText = string.Format(ModTranslation.getString("lawyerMeetingButton"),
                    Lawyer.neededMeetings - Lawyer.meetings);
            }
            ...
            if (__instance.state == 1)
            {
                ...
                int remaining = Mathf.Min(localRemaining,
                    Mayor.mayor != null && Mayor.mayor == PlayerControl.LocalPlayer ? 1 : teamRemaining);

                __instance.StatusText.text = "<size=100%>" + string.Format(ModTranslation.getString("meetingStatus"),
                    PlayerControl.LocalPlayer.name) + "</size>";
                __instance.NumberText.text = string.Format(ModTranslation.getString("meetingCount"),
                    localRemaining.ToString(), teamRemaining.ToString());
    ...
    public static class ConsolePatch
    {
        ...
        {
            public static bool Prefix(ref float __result, Console __instance,
                [HarmonyArgument(0)] NetworkedPlayerInfo pc, [HarmonyArgument(1)] out bool canUse,
    ...
    public class SystemConsolePatch
    {
        ...
        {
            public static bool Prefix(ref float __result, SystemConsole __instance,
                [HarmonyArgument(0)] NetworkedPlayerInfo pc, [HarmonyArgument(1)] out bool canUse,
                [HarmonyArgument(2)] out bool couldUse)
            {
                canUse = couldUse = false;
