...
public static class OnGameEndPatch
{
    ...
    {
        LogHelper.Info("-----------Game Ended-----------", "Phase");
        ...
        if (isFoxAlive && isFoxCompletedTasks)
        {
            ...
            // 第三陣営の勝利以外の場合に狐が生存していたら狐の勝ち
                     ...
                     not GameOverReason.HumansByTask and
                     not GameOverReason.ImpostorBySabotage)
                gameOverReason = (GameOverReason)CustomGameOverReason.FoxWin;
        }
        ...
        RoleType[] excludeRoles = new[] { RoleType.Lovers };
        foreach (NetworkedPlayerInfo p in GameData.Instance.AllPlayers)
        {
            //var p = pc.Data;
            List<RoleInfo> roles = RoleInfo.getRoleInfoForPlayer(p.Object, excludeRoles, true);
            (int tasksCompleted, int tasksTotal) = TasksHandler.taskInfo(p);
            FinalStatus finalStatus = finalStatuses[p.PlayerId] =
        ...
        AdditionalTempData.isGM =
            CustomOptionHolder.gmEnabled.getBool() && PlayerControl.LocalPlayer.isGM();
        ...
        bool impostorWin = gameOverReason == GameOverReason.ImpostorByKill ||
        ...
        bool lawyerSoloWin = Lawyer.lawyer != null &&
                             gameOverReason == (GameOverReason)CustomGameOverReason.LawyerSoloWin;
        bool plagueDoctorWin =
            PlagueDoctor.exists && gameOverReason == (GameOverReason)CustomGameOverReason.PlagueDoctorWin;
        ...
        bool jekyllAndHydeWin = JekyllAndHyde.exists &&
        ...
        else if (arsonistWin)
        {
            ...
            foreach (PlagueDoctor pd in PlagueDoctor.players)
            {
                EndGameResult.CachedWinners = new Il2CppSystem.Collections.Generic.List<CachedPlayerData>();
                CachedPlayerData wpd = new(pd.player.Data);
                EndGameResult.CachedWinners.Add(wpd);
        ...
        {
            EndGameResult.CachedWinners = new Il2CppSystem.Collections.Generic.List<CachedPlayerData>();
            foreach (Puppeteer puppeteer in Puppeteer.players)
            {
                CachedPlayerData wpd = new(puppeteer.player.Data);
                EndGameResult.CachedWinners.Add(wpd);
            }
        ...
        {
            EndGameResult.CachedWinners = new Il2CppSystem.Collections.Generic.List<CachedPlayerData>();
            foreach (JekyllAndHyde jekyllAndHyde in JekyllAndHyde.players)
            {
                CachedPlayerData wpd = new(jekyllAndHyde.player.Data);
                EndGameResult.CachedWinners.Add(wpd);
            }

            if (SchrodingersCat.team == SchrodingersCat.Team.JekyllAndHyde)
                foreach (PlayerControl schrodingersCat in SchrodingersCat.allPlayers)
                {
                    CachedPlayerData wpd = new(schrodingersCat.Data);
                    EndGameResult.CachedWinners.Add(wpd);
                }
        ...
        else if (moriartyWin)
        {
            ...
            {
                EndGameResult.CachedWinners = new Il2CppSystem.Collections.Generic.List<CachedPlayerData>();
                foreach (Moriarty moriarty in Moriarty.players)
                {
                    CachedPlayerData wpd = new(moriarty.player.Data);
                    EndGameResult.CachedWinners.Add(wpd);
                }
        ...
        else if (akujoWin)
        {
            ...
            foreach (Akujo akujo in Akujo.players)
            {
                ...
                {
                    EndGameResult.CachedWinners.Add(new CachedPlayerData(akujo.player.Data));
                    EndGameResult.CachedWinners.Add(new CachedPlayerData(akujo.honmei.player.Data));
                }

                if (akujo.player.isAlive() && akujo.cupidHonmei != null && akujo.cupidHonmei.isAlive())
                {
                    EndGameResult.CachedWinners.Add(new CachedPlayerData(akujo.player.Data));
                    EndGameResult.CachedWinners.Add(new CachedPlayerData(akujo.cupidHonmei.Data));
                }
            }
        }
        ...
        else if (teamJackalWin)
        {
            ...
            foreach (PlayerControl player in Jackal.formerJackals)
            {
            ...
            if (SchrodingersCat.team == SchrodingersCat.Team.Jackal)
                foreach (PlayerControl player in SchrodingersCat.allPlayers)
                {
                    CachedPlayerData wpdSchrodingersCat = new(player.Data)
                    {
                        IsImpostor = false
                    };
        ...
        {
            EndGameResult.CachedWinners = new Il2CppSystem.Collections.Generic.List<CachedPlayerData>();
            foreach (Fox fox in Fox.players)
            {
                CachedPlayerData wpd = new(fox.player.Data);
                EndGameResult.CachedWinners.Add(wpd);
            }

            foreach (Immoralist immoralist in Immoralist.players)
            {
                CachedPlayerData wpd = new(immoralist.player.Data);
                EndGameResult.CachedWinners.Add(wpd);
            }
        ...
        // キューピッドが悪女と勝利する
        if (akujoWin)
            foreach (Cupid p in Cupid.players)
            {
                if (p.player.isDead()) continue;
                if ((p.lovers1 != null && p.lovers1.isRole(RoleType.Akujo)) ||
                    (p.lovers2 != null && p.lovers2.isRole(RoleType.Akujo)))
                {
        ...
        // キューピッドがLoversと勝利する
            ...
            foreach (Cupid cupid in Cupid.players)
                if ((cupid.lovers1 != null) & cupid.lovers1.isAlive() && cupid.lovers2 != null &&
                    cupid.lovers2.isAlive())
                {
        ...
        {
            bool oppWin = false;
            foreach (PlayerControl p in Opportunist.livingPlayers)
            {
        ...
        foreach (CachedPlayerData wpd in EndGameResult.CachedWinners)
            wpd.IsDead = wpd.IsDead ||
                         AdditionalTempData.playerRoles.Any(x =>
                             x.PlayerName == wpd.PlayerName && x.Status != FinalStatus.Alive);
    ...
    public class EndGameNavigationPatch
    {
        ...
        public class EndGameManagerSetUpPatch
        {
            public static void Postfix(EndGameManager __instance)
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    poolablePlayer.cosmetics.nameText.text = cachedPlaterData2.PlayerName;

                    foreach (AdditionalTempData.PlayerRoleInfo data in AdditionalTempData.playerRoles)
                    {
                        if (data.PlayerName != cachedPlaterData2.PlayerName) continue;
                ...
                bonusTextObject.transform.position = new Vector3(__instance.WinText.transform.position.x,
                ...
                else if (AdditionalTempData.gameOverReason is GameOverReason.ImpostorByKill
                         or GameOverReason.ImpostorBySabotage or GameOverReason.ImpostorByVote)
                {
                ...
                if (TORMapOptions.showRoleSummary)
                {
                    ...
                    roleSummary.transform.position =
                        new Vector3(__instance.Navigation.ExitButton.transform.position.x + 0.1f, position.y - 0.1f,
                            -14f);
                    ...
                    AdditionalTempData.playerRoles.Sort((x, y) =>
                    {
                        ...
                        {
                            if (idX == idY) return x.PlayerName.CompareTo(y.PlayerName);
                            return idX.CompareTo(idY);
                        }

                        return x.Status.CompareTo(y.Status);
                    });
                    LogHelper.Info(textRenderer.text, "Result");
                    bool plagueExists =
                        AdditionalTempData.playerRoles.Any(x => x.Roles.Contains(RoleInfo.plagueDoctor));
                    LogHelper.Info("----------Game Result-----------", "Result");
                    foreach (AdditionalTempData.PlayerRoleInfo data in AdditionalTempData.playerRoles)
                    {
                        if (data.PlayerName == "") continue;
                        string taskInfo = data.TasksTotal > 0
                        ...
                        string result =
                            $"{data.PlayerName + data.NameSuffix}<pos=18.5%>{taskInfo}<pos=25%>{aliveDead}<pos=34%>{data.RoleString}";
                        if (plagueExists && !data.Roles.Contains(RoleInfo.plagueDoctor))
                        {
                            ...
                            {
                                float progress = AdditionalTempData.plagueDoctorProgress.ContainsKey(data.PlayerId)
                                    ? AdditionalTempData.plagueDoctorProgress[data.PlayerId]
                                    : 0f;
                    ...
                    if (AmongUsClient.Instance.AmHost)
                    {
                        ...
                        foreach (AdditionalTempData.PlayerRoleInfo data in AdditionalTempData.playerRoles)
                        {
                            ...
                            string taskInfo = string.Format("{0:D2}", data.TasksCompleted) + "/" +
                            ...
                            if (plagueExists && !data.Roles.Contains(RoleInfo.plagueDoctor))
                            {
                                result += " | ";
                                if (AdditionalTempData.plagueDoctorInfected.ContainsKey(data.PlayerId))
                                    result += Helpers.cs(Color.red,
                                        ModTranslation.getString("plagueDoctorInfectedText"));
                                else
                                {
                                    float progress = AdditionalTempData.plagueDoctorProgress.ContainsKey(data.PlayerId)
                                        ? AdditionalTempData.plagueDoctorProgress[data.PlayerId]
        ...
        private class CheckEndCriteriaPatch
        {
            ...
            private static bool CheckAndEndGameForJekyllAndHydeWin(ShipStatus __instance, PlayerStatistics statistics)
            {
                ...
                if (statistics.JekyllAndHydeAlive >=
            ...
            private static bool CheckAndEndGameForMoriartyWin(ShipStatus __instance, PlayerStatistics statistics)
            {
                ...
                if (statistics.MoriartyAlive >=
                    statistics.TotalAlive - statistics.MoriartyAlive - statistics.FoxAlive &&
                    statistics.TeamImpostorsAlive == 0 && statistics.TeamJackalAlive == 0 &&
            ...
            private static bool CheckAndEndGameForTaskWin(ShipStatus __instance)
            {
                ...
                if (Fox.exists && !Fox.crewWinsByTasks)
                {
                    ...
                    if (isFoxCompletedtasks && isFoxAlive && GameData.Instance.TotalTasks > 0 &&
                        GameData.Instance.TotalTasks <=
                        GameData.Instance.CompletedTasks + numDeadPlayerUncompletedTasks)
                    {
                        UncheckedEndGame(GameOverReason.HumansByTask);
                        return true;
                    }
            ...
            {
                if (statistics.TeamImpostorsAlive >=
            ...
            private static bool CheckAndEndGameForCrewmateWin(ShipStatus __instance, PlayerStatistics statistics)
            {
                if (statistics.TeamCrew > 0 && statistics.TeamImpostorsAlive == 0 && statistics.TeamJackalAlive == 0 &&
                    statistics.JekyllAndHydeAlive == 0 && statistics.MoriartyAlive == 0)
                {
        ...
        internal class PlayerStatistics
        {
            ...
            private void GetPlayerCounts()
            {
                ...
                if (Godfather.godfather?.isDead() == true && Mafioso.mafioso?.isDead() == true &&
                    Janitor.janitor?.isDead() == false) numImpostorsAlive = 0;

                // 爆弾魔を一人としてカウントする、猫の自爆中はインポスターのカウントを一人減らす
                // PlayerControl.isAlive()を使うと会議での追放時にカウントバグが発生するので使用禁止
                if (SchrodingersCat.killer != null &&
                    !(SchrodingersCat.killer.Data.IsDead || SchrodingersCat.killer.Data.Disconnected) &&
                    SchrodingersCat.team == SchrodingersCat.Team.Impostor)
                    numImpostorsAlive--;
                ...
                if (SchrodingersCat.killer != null &&
                    !(SchrodingersCat.killer.Data.IsDead || SchrodingersCat.killer.Data.Disconnected) &&
                    SchrodingersCat.team == SchrodingersCat.Team.Jackal) numJackalAlive--;
                if (SchrodingersCat.livingPlayers.Count > 0 &&
                    SchrodingersCat.team == SchrodingersCat.Team.JekyllAndHyde)
                ...
                if (SchrodingersCat.killer != null &&
                    !(SchrodingersCat.killer.Data.IsDead || SchrodingersCat.killer.Data.Disconnected) &&
                    SchrodingersCat.team == SchrodingersCat.Team.JekyllAndHyde) --numJekyllAndHydeAlive;

                if (SchrodingersCat.killer != null &&
                    !(SchrodingersCat.killer.Data.IsDead || SchrodingersCat.killer.Data.Disconnected) &&
                    SchrodingersCat.team == SchrodingersCat.Team.Moriarty) --numMoriartyAlive;

                // 人形使いのダミーはカウントしない
                if (Puppeteer.dummy != null) numTotalAlive--;

                // モリアーティに洗脳されているユーザーはモリアーティー陣営としてカウントする
                if (Moriarty.target != null)
                {
                    if (Moriarty.target.isImpostor() || (Moriarty.target.isRole(RoleType.SchrodingersCat) &&
                                                         SchrodingersCat.team == SchrodingersCat.Team.Impostor))
                        numImpostorsAlive -= 1;
                    else if (Moriarty.target.isRole(RoleType.Jackal) || Moriarty.target.isRole(RoleType.Sidekick) ||
                             (Moriarty.target.isRole(RoleType.SchrodingersCat) &&
                              SchrodingersCat.team == SchrodingersCat.Team.Jackal)) numJackalAlive -= 1;
                    else if (Moriarty.target.isRole(RoleType.JekyllAndHyde) ||
                             (Moriarty.target.isRole(RoleType.SchrodingersCat) &&
                              SchrodingersCat.team == SchrodingersCat.Team.JekyllAndHyde)) numJekyllAndHydeAlive -= 1;
                    else numCrew -= 1;
                    numMoriartyAlive += 1;
                }
