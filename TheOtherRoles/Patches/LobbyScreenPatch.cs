...
public sealed class LobbyJoinBind
{
    ...
    public static void Postfix()
    {
        if (!LobbyText)
        {
            ...
            TextMeshPro comp = LobbyText.AddComponent<TextMeshPro>();
            comp.fontSize = 2.5f;
        ...
        if (LobbyText)
        {
            ...
            if (GameId != 0 && GameId != 32)
            {
                ...
                if (code != "")
                {
                    ...
                    LobbyText.GetComponent<TextMeshPro>().text =
                        ModTranslation.getString("lobbyJoinBindText1") + $" {code}   [LShift]";
                }
            }

            if (code2 != "")
                LobbyText.GetComponent<TextMeshPro>().text +=
                    ModTranslation.getString("lobbyJoinBindText2") + $" {code2Disp}  [RShift]";
        }
    }
}
