...
internal class ExileControllerBeginPatch
{
    ...
    public static void Prefix(ExileController __instance, [HarmonyArgument(0)] ref NetworkedPlayerInfo exiled)
    {
        ...
        if (Witch.witch != null && Witch.futureSpelled != null && AmongUsClient.Instance.AmHost)
        {
            ...
            bool witchDiesWithExiledLover = exiled != null && Lovers.bothDie && exiled.Object.isLovers() &&
                                            exiled.Object.getPartner() == Witch.witch;
    ...
    private static PlayerControl pickRandomCrewmate(int exiledPlayerId)
    {
        ...
        int targetPlayerIndex = rnd.Next(0, numAliveCrewmates);
...
internal class ExileControllerWrapUpPatch
{
    ...
    private static void WrapUpPostfix(PlayerControl exiled)
    {
        ...
        if (exiled != null)
        {
            PlayerControl p = Helpers.playerById(exiled.PlayerId);
            if (p.hasModifier(ModifierType.Mini) && !Mini.isGrownUp(p) && !p.Data.Role.IsImpostor && !p.isNeutral())
                Mini.triggerMiniLose = true;

            // Jester win condition
            else if (p.isRole(RoleType.Jester)) Jester.triggerJesterWin = true;
        }
        ...
        LogHelper.Info("-----------Task Start-----------", "Phase");
    }
...
internal class ExileControllerReEnableGameplayPatch
{
    ...
    public static void ReEnableGameplay()
    {
        ...
        if (Medium.medium != null && PlayerControl.LocalPlayer == Medium.medium)
        {
            ...
            {
                foreach (SpriteRenderer sr in Medium.souls) Object.Destroy(sr.gameObject);
        ...
        if (Lawyer.lawyer != null && PlayerControl.LocalPlayer == Lawyer.lawyer &&
            !Lawyer.lawyer.Data.IsDead)
            Lawyer.meetings++;
        ...
        for (int i = 0; i < array.Length; i++) Object.Destroy(array[i].gameObject);

        // ベントバグ対策
        VentilationSystem vs = FastDestroyableSingleton<ShipStatus>.Instance.Systems[SystemTypes.Ventilation]
    ...
    private static void Postfix(ref string __result, [HarmonyArgument(0)] StringNames id)
    {
        try
        {
            if (ExileController.Instance != null && ExileController.Instance.initData != null)
            {
                ...
                // Exile role text
                if (id is StringNames.ExileTextPN or StringNames.ExileTextSN or StringNames.ExileTextPP
