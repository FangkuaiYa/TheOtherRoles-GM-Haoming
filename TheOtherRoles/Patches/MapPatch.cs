...
internal class MapBehaviorPatch
{
    ...
    public static void reset()
    {
        ...
        {
            foreach (SpriteRenderer r in impostorHerePoint.Values) Object.Destroy(r.gameObject);
        ...
        if (doorMarks != null)
        {
            foreach (SpriteRenderer mark in doorMarks.Values) Object.Destroy(mark.gameObject);
            doorMarks.Clear();
            doorMarks = null;
        }
    ...
    private static void initializeIcons(MapBehaviour __instance, PlayerControl pc = null)
    {
        ...
        if (pc == null)
        {
            ...
            foreach (PlayerControl p in PlayerControl.AllPlayerControls) players.Add(p);
        }
        else
            players.Add(pc);
    ...
    private static void showDoorStatus(MapBehaviour __instance)
    {
        ...
        {
            foreach (SpriteRenderer mark in doorMarks.Values) mark.gameObject.SetActive(false);
            return;
        }
        ...
        if (doorMarks == null) doorMarks = new Dictionary<string, SpriteRenderer>();

        foreach (PlainDoor door in plainDoors)
        {
    ...
    private static void evilTrackerFixedUpdate(MapBehaviour __instance)
    {
        ...
        if (EvilTracker.target != null)
        {
            if (targetHerePoint == null)
                targetHerePoint = GameObject.Instantiate(__instance.HerePoint, __instance.HerePoint.transform.parent);
            ...
            Vector3 pos = new(EvilTracker.target.transform.position.x, EvilTracker.target.transform.position.y,
                EvilTracker.target.transform.position.z);
        ...
        // インポスターの位置をマップに表示
        if (impostorHerePoint == null) impostorHerePoint = new Dictionary<byte, SpriteRenderer>();
        foreach (PlayerControl p in PlayerControl.AllPlayerControls)
            if (p.isImpostor() && p != PlayerControl.LocalPlayer)
            {
                if (!impostorHerePoint.ContainsKey(p.PlayerId))
                    impostorHerePoint[p.PlayerId] =
                        GameObject.Instantiate(__instance.HerePoint, __instance.HerePoint.transform.parent);
    ...
    private static bool evilTrackerShowMap(MapBehaviour __instance)
    {
        ...
        if (TheOtherRolesPlugin.HideFakeTasks.Value &&
            !(PlayerControl.LocalPlayer.isRole(RoleType.EvilTracker) && EvilTracker.target != null))
    ...
    private static bool evilHackerShowMap(MapBehaviour __instance)
    {
        ...
        __instance.infectedOverlay.gameObject.SetActive(MeetingHud.Instance ? false : true);
        if (MeetingHud.Instance != null && PlayerControl.LocalPlayer.isRole(RoleType.EvilTracker) &&
            EvilTracker.canSeeTargetTask)
            __instance.taskOverlay.Show();
    ...
    private static bool evilTrackerShowTask(MapTaskOverlay __instance)
    {
        if (!MeetingHud.Instance)
            return
                true; // Only run in meetings, and then set the Position of the HerePoint to the Position before the Meeting!
        if (!PlayerControl.LocalPlayer.isRole(RoleType.EvilTracker) ||
            !CustomOptionHolder.evilTrackerCanSeeTargetTask.getBool()) return true;
        ...
        for (int i = 0; i < realTasks[EvilTracker.target.PlayerId].Count; i++)
            try
            {
                ...
                pooledMapIcon.transform.localScale = new Vector3(pooledMapIcon.NormalSize, pooledMapIcon.NormalSize,
    ...
    private class MapBehaviourFixedUpdatePatch
    {
        ...
        private static void Postfix(MapBehaviour __instance)
        {
            ...
            if (PlayerControl.LocalPlayer.isGM())
            {
                foreach (PlayerControl p in PlayerControl.AllPlayerControls)
                {
                    ...
                    byte id = p.PlayerId;
                    if (!mapIcons.ContainsKey(id)) continue;
                ...
                foreach (DeadBody b in Object.FindObjectsOfType<DeadBody>())
                {
                    ...
                    if (!corpseIcons.ContainsKey(id)) continue;
    ...
    private class MapBehaviourShowNormalMapPatch
    {
        private static bool Prefix(MapBehaviour __instance)
        {
            if (!MeetingHud.Instance || __instance.IsOpen)
                return true; // Only run in meetings and when the map is closed
    ...
    private class MapBehaviourGenericShowPatch
    {
    ...
    private class MapBehaviorClosePatch
    {
        private static void Postfix(MapBehaviour __instance)
        {
            if (PlayerControl.LocalPlayer.isGM())
    ...
    private class MapBehaviorGetIsOpenStoppedPatch
    {
        private static bool Prefix(ref bool __result, MapBehaviour __instance)
        {
            if ((PlayerControl.LocalPlayer.isRole(RoleType.EvilHacker) || EvilHacker.isInherited()) &&
    ...
    private class MapBehaviourShowSabotageMap
    {
        private static bool Prefix(MapBehaviour __instance)
        {
        ...
        private static void Postfix(MapBehaviour __instance)
        {
            if (TheOtherRolesPlugin.HideFakeTasks.Value) __instance.taskOverlay.Hide();
        }
    }

    [HarmonyPatch(typeof(MapTaskOverlay), nameof(MapTaskOverlay.Show))]
    private class MapTaskOverlayShow
    {
...
internal class MapCountOverlayEnablePatch
{
    public static bool Prefix(MapCountOverlay __instance)
    {
        if (CustomOptionHolder.impostorCanIgnoreComms.getBool() &&
            PlayerControl.LocalPlayer.isImpostor()) return false;
        return true;
    }
}

[HarmonyPatch(typeof(MapCountOverlay), nameof(MapCountOverlay.Update))]
internal class MapCountOverlayUpdatePatch
{
    public static bool Prefix(MapCountOverlay __instance)
    {
        if (CustomOptionHolder.impostorCanIgnoreComms.getBool() && PlayerControl.LocalPlayer.isImpostor())
        {
            ...
            for (int i = 0; i < __instance.CountAreas.Length; i++)
            {
                ...
                if (ShipStatus.Instance.FastRooms.TryGetValue(counterArea.RoomType, out plainShipRoom) &&
...
internal class AlphaPuluseUpdatePatch
{
    public static void Postfix(AlphaPulse __instance)
    {
        if (!TheOtherRolesPlugin.TransparentMap.Value) return;
        if (__instance.rend)
            __instance.rend.color = new Color(__instance.rend.color.r, __instance.rend.color.g, __instance.rend.color.b,
                0.2f);
        if (__instance.mesh)
            __instance.mesh.material.color = new Color(__instance.mesh.material.color.r,
                __instance.mesh.material.color.g, __instance.mesh.material.color.b, 0.2f);
    }
}
