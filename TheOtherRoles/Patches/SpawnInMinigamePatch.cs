...
public class SpawnInMinigamePatch
{
    ...
    private static PassiveButton selected;
    ...
    public static float initialDoorCooldown => CustomOptionHolder.airshipInitialDoorCooldown.getFloat();
    public static float initialSabotageCooldown => CustomOptionHolder.airshipInitialSabotageCooldown.getFloat();
    ...
    public static void resetSpawnCandidates()
    {
        ...
        {
            SpawnCandidates.Add(new SpawnCandidate(StringNames.VaultRoom, new Vector2(-8.8f, 8.6f),
                "TheOtherRoles.Resources.Locations.VaultButton.png", "rollover_brig"));
            SpawnCandidates.Add(new SpawnCandidate(StringNames.MeetingRoom, new Vector2(11.0f, 14.7f),
                "TheOtherRoles.Resources.Locations.MeetingButton.png", "rollover_brig"));
            SpawnCandidates.Add(new SpawnCandidate(StringNames.Cockpit, new Vector2(-22.0f, -1.2f),
                "TheOtherRoles.Resources.Locations.CockpitButton.png", "rollover_brig"));
            SpawnCandidates.Add(new SpawnCandidate(StringNames.Electrical, new Vector2(16.4f, -8.5f),
                "TheOtherRoles.Resources.Locations.ElectricalButton.png", "rollover_brig"));
            SpawnCandidates.Add(new SpawnCandidate(StringNames.Lounge, new Vector2(30.9f, 7.5f),
                "TheOtherRoles.Resources.Locations.LoungeButton.png", "rollover_brig"));
            SpawnCandidates.Add(new SpawnCandidate(StringNames.Medical, new Vector2(25.5f, -5.0f),
                "TheOtherRoles.Resources.Locations.MedicalButton.png", "rollover_brig"));
            SpawnCandidates.Add(new SpawnCandidate(StringNames.Security, new Vector2(10.3f, -16.2f),
                "TheOtherRoles.Resources.Locations.SecurityButton.png", "rollover_brig"));
            SpawnCandidates.Add(new SpawnCandidate(StringNames.ViewingDeck, new Vector2(-14.1f, -16.2f),
                "TheOtherRoles.Resources.Locations.ViewingButton.png", "rollover_brig"));
            SpawnCandidates.Add(new SpawnCandidate(StringNames.Armory, new Vector2(-10.7f, -6.3f),
                "TheOtherRoles.Resources.Locations.ArmoryButton.png", "rollover_brig"));
            SpawnCandidates.Add(new SpawnCandidate(StringNames.Comms, new Vector2(-11.8f, 3.2f),
                "TheOtherRoles.Resources.Locations.CommunicationsButton.png", "rollover_brig"));
            SpawnCandidates.Add(new SpawnCandidate(StringNames.Showers, new Vector2(20.8f, 2.8f),
                "TheOtherRoles.Resources.Locations.ShowersButton.png", "rollover_brig"));
            SpawnCandidates.Add(new SpawnCandidate(StringNames.GapRoom, new Vector2(13.8f, 6.4f),
                "TheOtherRoles.Resources.Locations.GapButton.png", "rollover_brig"));
            foreach (SpawnCandidate spawnCandidate in SpawnCandidates) spawnCandidate.ReloadTexture();
        }
    }
    ...
    public static bool Prefix(SpawnInMinigame __instance, PlayerTask task)
    {
        ...
        foreach (SpawnCandidate spawnCandidate in SpawnCandidates)
        {
        ...
        for (int i = 0; i < __instance.LocationButtons.Length; i++)
        {
            ...
            passiveButton.GetComponentInChildren<TextMeshPro>().text =
                FastDestroyableSingleton<TranslationController>.Instance.GetString(pt.Name,
        ...
        ControllerManager.Instance.OpenOverlayMenu(__instance.name, null, __instance.DefaultButtonSelected,
    ...
    public static void Postfix(SpawnInMinigame __instance)
    {
        ...
        if (!CustomOptionHolder.airshipSynchronizedSpawning.getBool() ||
            CustomOptionHolder.airshipRandomSpawn.getBool()) return;
    ...
    {
        if (!CustomOptionHolder.airshipSynchronizedSpawning.getBool() ||
            CustomOptionHolder.airshipRandomSpawn.getBool())
        {
                ...
                foreach (PassiveButton button in __instance.LocationButtons)
                {
                    if (selected == button)
                    {
                        if (time > 0.3f)
                        {
                            ...
                            button.transform.localPosition = new Vector3(x, button.transform.localPosition.y,
                    ...
                    if (synchronizeData.Align(SynchronizeTag.PreSpawnMinigame, false) || p == 1f)
                    {
                        ...
                        CustomButton.stopCountdown = false;
                        // サボタージュのクールダウンをリセット
                        SabotageSystemType saboSystem = MapUtilities.CachedShipStatus.Systems[SystemTypes.Sabotage]
                            .Cast<SabotageSystemType>();
                        AccessTools.PropertySetter(typeof(SabotageSystemType), "IsDirty").Invoke(saboSystem,
                            new object[]
                            {
                                true
                            });
                        saboSystem._Timer_k__BackingField = 0f;
                        saboSystem.Timer = initialSabotageCooldown;
                        DoorsSystemType doorSystem = MapUtilities.CachedShipStatus.Systems[SystemTypes.Doors]
                            .Cast<DoorsSystemType>();
